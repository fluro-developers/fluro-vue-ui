...
export default {
	...
	computed: {
		...
		reloadRequired() {
			return `${this.cacheKey}-${this.searchInheritable}-${this.dataType}-${this.joins}-${
				this.filterCheckString
			} ${this.dateWatchString} ${this.sort.sortKey} ${this.sort.sortDirection} ${this.sort.sortType} ${
				this.groupingColumn ? this.groupingColumn.key : ''
			} ${this.unwindColumns && this.unwindColumns.length ? _.map(this.unwindColumns, 'key').join('.') : ''}  ${
