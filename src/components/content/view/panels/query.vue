					...
					<flex-column-body style="background: #fff">
						<v-container fluid>
							<constrain sm>
								...
								<p>
									This query can be retrieved and rendered in different ways. Select below to get a
									templated feed of it's results.
								</p>
...
export default {
	...
	methods: {
		...
		exportCSV() {
			...
			self.$fluro.api.post(`/export/query/${self.item._id}/csv`).then(
				function (res) {
	...
	asyncComputed: {
		results: {
			...
			get() {
				...
				return new Promise(function (resolve, reject) {
					self.$fluro.content
						.query(self.item, { cb }, { cache: false })
		...
		dataSets: {
			...
			get() {
				...
				return new Promise(function (resolve, reject) {
					self.$fluro.api
						.get(`/query/${self.$fluro.utils.getStringID(self.item)}/resultsets/metadata`, { cache: false })
						.then(function (res) {
