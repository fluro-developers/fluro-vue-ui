...
export default {
	...
	methods: {
		initMaps() {
			...
			self.$fluro.utils
				.injectScript(`https://maps.googleapis.com/maps/api/js?key=${self.$fluro.global.googleMapsAPIKey}`)
				.then(function (res) {
					...
					if (self.floatPositions.length) {
						myCenter = new google.maps.LatLng(
							self.floatPositions[0].latitude,
							self.floatPositions[0].longitude
						);
					} else {
		...
		createContentString(obj) {
			...
			text +=
				"</p><button type='button' onClick='openViewModal(`" +
				obj._id +
				"`)' class='v-btn v-btn--flat theme--light'><div class='v-btn__content'>View</div></button><button type='button' onClick='openEditModal(`" +
				obj._id +
				"`)' class='v-btn v-btn--flat theme--light'><div class='v-btn__content'>Edit</div></button></div>";
			return text;
		},
	},
	watch: {
