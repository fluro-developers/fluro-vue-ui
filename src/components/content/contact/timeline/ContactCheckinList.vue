                                ...
                                <div class="entries">
                                    <div @click="$fluro.global.view(entry, true)" class="entry" v-for="entry in day.items" :key="entry._id">
                                        ...
                                        <v-layout align-center>
                                            <v-flex shrink>
                                                <div class="thumbnail" :style="{backgroundImage:`url(${$fluro.asset.coverUrl(entry.event._id, `event`, {w:50})})`}" />
                                            </v-flex>
                                            <v-flex>
                                                <div>
                                                <strong>{{entry.event.title}}</strong>
                                                <div><strong class="sm">{{entry.event | readableEventDate}}</strong></div>
                                                <div class="sm text-muted">{{ (entry.event.definition ? entry.event.definition : 'event') | definitionTitle}}  {{entry.event.firstLine}}</div>
                                                </div>
                                                <!-- <pre>{{entry}}</pre> -->
