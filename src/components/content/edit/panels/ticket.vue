		...
		<tabset v-else :justified="true" :vertical="true">
			<tab>
				<flex-row>
							...
							<constrain sm>
								<v-layout column grid-list-xl>
									...
									<fluro-content-form-field
										...
										@input="update"
										:options="options"
										:field="fieldHash.interaction"
										v-model="model"
									></fluro-content-form-field>
									<fluro-panel>
										<fluro-panel-title>
											<div class="no-border-no-background" @click="toggleCollected()">
												<fluro-toggle-item :inactive="!model.collected"
													>Ticket has been collected</fluro-toggle-item
												>
											</div>
										</fluro-panel-title>
											...
											<v-layout row wrap>
												...
												<v-flex xs-12 sm6 px-1>
													<fluro-content-form-field
														...
														@input="update"
														:options="options"
														:field="fieldHash.collectionDate"
														v-model="model"
													></fluro-content-form-field>
												</v-flex>
											</v-layout>
					...
					<flex-column
						v-if="qrCodeURL"
						style="max-width: 300px; width: 25%; background: #eee"
						class="border-left"
					>
						<v-container fluid>
