			...
			<tabset v-model="tabIndex" :justified="true">
				...
				<tab heading="Basic Permissions" index="basic">
					<tabset :justified="true" :vertical="true" :persist="true">
						...
						<tab
							:heading="`${type.title} ${of(type)}`"
							:muted="!enabled(type).length"
							:key="type.title"
							v-for="type in filteredBasic"
						>
							<flex-column-body>
								<v-container>
									...
									<list-group>
										<fluro-toggle-item
											@click.native="togglePermission(permission)"
											:key="permission.title"
											v-for="permission in type.permissions"
											:inactive="!permissionSelected(permission)"
										>
	...
	computed: {
		...
		fieldsOutput() {
			...
			addField('firstLine', {
				title: 'Description',
				description:
					'Enter a one line description summarizing what this role is for and the permissions it grants',
				minimum: 0,
				maximum: 1,
				type: 'string',
			});
