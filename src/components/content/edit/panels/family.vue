                        ...
                        <v-container fluid grid-list-lg>
                            ...
                            <v-layout row wrap fill-height style="overflow-x:scroll">
                                <div v-for="(contact, index) in members" :key="contact._id" class="cards border">
                                    <family-member-card @delete="deleteContact" :contactDefinitions="contactDefinitionOptions" v-model="members[index]" />
                                </div>
...
export default {
    ...
    computed: {
        ...
        fieldsOutput() {
            ...
            addField("items", {
                title: self.model._context == "contact" ?
                    "Additional Family Member" : "Family Member",
    ...
    methods: {
        ...
        create() {
            ...
            self.$fluro.options(options, 'New or existing?', `Please select an option below`)
                .then(function(answer) {
                    ...
                    switch (answer.key) {
                        case 'existing':
                            ...
                            self.$fluro.global.select('contact', {
                                    allDefinitions: true,
                                    minimum: 0,
                                    maximum: 0,
                                }, true)
                                .then(function(contacts) {
                                    ...
                                    self.$fluro.api.put(`/family/${self.model._id}/add`, {
                                            contacts: contactIDs,
                                        })
                                        .then(function(res) {
                                            ...
                                            var reduced = _.reduce(existingContactIDs, function(set, id) {
                                                set[id] = true;
                                                return set;
                                            }, {});
                                            ...
                                            self.$fluro.notify(`${newData.items.length} contacts added to '${self.model.title}'`);
                                        })
                                })
