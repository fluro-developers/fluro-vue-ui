										...
										<v-input class="no-flex" label="Contact">
											<v-layout align-center>
												...
												<v-flex>
													...
													<div class="muted">
														{{ model.definition || 'contact' | definitionTitle }}
													</div>
												</v-flex>
												<v-flex shrink style="padding-right: 10px">
													<v-btn
														class="ma-0"
														v-if="$fluro.access.canEditItem(model.contact)"
														icon
														@click="$fluro.global.edit(model.contact, true)"
													>
													...
													<v-btn
														class="ma-0 ml-1"
														icon
														@click="$actions.open([model.contact])"
													>
										...
										<v-input class="no-flex" label="User Persona">
											<v-layout align-center>
												...
												<v-flex>
													...
													<div v-if="model.persona.collectionEmail">
														{{ model.persona.collectionEmail }}
													</div>
													<div v-else="model.persona.userEmail">
														{{ model.persona.userEmail }}
													</div>
													<div v-if="model.persona.username">
														{{ model.persona.username }}
													</div>
													<div class="muted">
														{{ model.definition || 'persona' | definitionTitle }}
													</div>
												</v-flex>
												<v-flex shrink style="padding-right: 10px">
													<v-btn
														class="ma-0"
														v-if="$fluro.access.canEditItem(model.persona)"
														icon
														@click="$fluro.global.edit(model.persona, true)"
													>
														<fluro-icon icon="pencil" />
													</v-btn>
													<v-btn
														class="ma-0 ml-1"
														icon
														@click="$actions.open([model.persona])"
													>
														<fluro-icon icon="ellipsis-h" />
													</v-btn>
												</v-flex>
								...
								<fluro-panel>
									...
									<fluro-panel-body class="border-top">
										...
										<v-input
											...
											:hint="`(${model.integration.module})`"
											persistent-hint
											v-if="model.integration"
										>
											<v-layout align-center>
												<v-flex>
								...
								<template v-if="model.module == 'pushpay'">
									<fluro-panel>
										<fluro-panel-title>
											...
											<strong v-else
												>Pushpay {{ model.transactionData.paymentMethodType }}</strong
											>
										</fluro-panel-title>
										<fluro-panel-body>
											<v-layout>
								...
								<template v-else>
									<fluro-panel>
										...
										<fluro-panel-body v-if="cardDetails">
											...
											<h5>
												{{ cardDetails.name }} {{ cardDetails.brand }} {{ cardDetails.country }}
											</h5>
											...
											<v-layout v-else>
												<v-flex xs12 sm6>
													<v-btn
														...
														large
														color="primary"
														@click="confirmRefund(model.total)"
													>
														Full Refund &nbsp; (<strong>{{ formattedTotal }}</strong
														>&nbsp; <Span class="currency">{{ model.currency }}</Span
														>)
													</v-btn>
												</v-flex>
												<v-spacer />
												<v-flex xs12 sm6>
													<v-btn block large @click="promptPartial()"> Partial Refund </v-btn>
												</v-flex>
											</v-layout>
										</fluro-panel-body>
...
export default {
	...
	computed: {
		...
		fieldsOutput() {
			...
			addField('email', {
				...
				maximum: 1,
				description:
					'The email address to be linked for this transaction (this is where receipts are sent to and is usually the email address of the payer)',
				type: 'email',
			});
	...
	methods: {
		...
		confirmRefund(amount) {
			...
			self.$fluro
				.confirm(
					`Are you sure you want to refund ${self.$fluro.utils.formatCurrency(amount, self.model.currency)}?`
				)
				.then(function (res) {
					self.refunding = true;

					self.$fluro.api
						.post(`/payment/refund/${self.model._id}`, { amount })
						.then(function (res) {
