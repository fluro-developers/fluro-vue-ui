							...
							<v-container>
								...
								<constrain sm>
									...
									<fluro-panel v-if="model.taskCount && model.taskCount.checkinsExpected">
										<fluro-panel-title class="border-bottom">
											<strong
												>{{ model.taskCount.checkinsCompleted }} of
												{{ model.taskCount.checkinsExpected }} checkins complete</strong
											>
									...
									<template v-if="model.forms && model.forms.length">
												...
												<list-group-item v-for="form in awaitingForms">
													...
													<template v-slot:right>
														...
														<v-btn
															...
															small
															icon
															@click="manualLink(form)"
															:loading="linking[form.form._id]"
															v-tippy
															content="Link to an existing form submission"
														>
														...
														<v-btn
															...
															color="primary"
															@click="resend(form)"
															:loading="resending[form.form._id]"
														>
															Resend
										...
										<v-input class="no-flex" v-if="receivedForms && receivedForms.length">
											<v-label
												>Received {{ receivedForms.length }} Form Response{{
													receivedForms.length == 1 ? '' : 's'
												}}</v-label
											>
											<!-- <p class="help-block">These forms have not yet been submitted</p> -->
											<!-- <list-group> -->
											<fluro-panel :key="index" v-for="(form, index) in receivedForms">
												...
												<fluro-panel-body style="padding: 0">
													...
													<fluro-content-view
														:id="form.interaction"
														:embedded="true"
														:definition="form.interaction.definition"
														type="interaction"
													/>
													<!-- </flex-column-body> -->
												</fluro-panel-body>
									...
									<template>
										...
										<!-- <template> -->
										<fluro-task-list
											@remove="removeTaskList(index)"
											@input="dispatchUpdate"
											:card="model"
											v-model="model.taskLists[index]"
											:state="model.state"
											:definition="definition"
											v-for="(list, index) in model.taskLists"
										></fluro-task-list>
										<!-- </template> -->
						...
						<flex-column-body>
							<v-container>
								...
								<v-input class="no-flex" v-if="showKeyContacts">
									...
									<p class="help-block">
										This card does not reference a contact, Here you can specify who should be sent
										Automated SMS and Mailouts as this card progresses
									</p>
									<fluro-content-form-field
										:form-fields="formFields"
										:showLabel="false"
...
export default {
	...
	methods: {
		manualLink(form) {
			...
			self.$fluro.global.select(form.form.definitionName, { maximum: 1 }).then(function (interactions) {
