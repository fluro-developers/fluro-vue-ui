	...
	<div>
		<fluro-panel>
			...
			<fluro-panel-body>
				<div class="integration-logo">
					...
					<div class="font-sm muted text-xs-center">
						By using the CCLI Song Select Service you agree to the <br /><a
							href="https://au.ccli.com/licence-terms/"
							target="_blank"
			...
			<fluro-panel-body class="border-top">
				...
				<p>
					<strong>By creating this integration you agree to the CCLI EULA Including all points below:</strong>
				</p>
				<ul>
					<li>
						All Content and the SongSelect Application are provided solely and exclusively for End Users who
						are active subscribers to SongSelect.
					</li>
					<li>
						All Content is the intellectual property of a third party, is protected by law, and no ownership
						or proprietary rights in the same are being transferred to Fluro and/or any End User
					</li>
					<li>CCL and/or Content Owners may remove content from the API from time-to-time without notice.</li>
					<li>
						The End User agrees to comply with all applicable laws, including copyright law in their use of
						the Content and shall protect the Content Owner's rights.
					</li>
					<li>
						The usage rights granted are nonexclusive, are subject to revocation or termination at any time,
						and may not be transferred, assigned, sublicensed, distributed or conveyed in any manner
						whatsoever.
					</li>
					<li>
						Neither CCLI not Content Owners will be liable for any indirect, punitive, incidental,
						consequential or special damages of any kind resulting from the use of the SongSelect API or
						from Content made available to Fluro through the SongSelect API, or for any damages or for any
						other damages of any type, nature or description arising out of any legal theory. This damage
						limitation shall be set forth in the EULA in conspicuous, bold, capitalized letters in
		...
		<fluro-panel v-if="false">
			<!-- !model._id"> -->
			<fluro-panel-body class="border-top"> Please save this integration to continue </fluro-panel-body>
		</fluro-panel>
		<template v-else>
				...
				<v-layout>
					<v-flex xs12 sm6>
						<v-btn
							...
							block
							large
							:to="{ name: 'songselect', params: { integration: model._id } }"
						>
							Browse Song Library
	...
	methods: {
		authorize(url) {
			...
			if (self.saveCallback) {
				return self.saveCallback(true).then(
					function (res) {
			...
			function redirectToURL(result) {
				var url = self.$fluro.api.generateEndpointURL(`/integrate/songselect/${self.model._id}/oauth`, {
					mode: self.model.publicDetails.mode || 'prod',
				});
				if (process.browser) {
