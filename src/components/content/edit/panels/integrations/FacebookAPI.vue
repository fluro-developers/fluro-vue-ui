<template>
	<div>
		<fluro-panel>
			<fluro-panel-title>
				<fluro-icon icon="facebook" library="fab" left /> Facebook Live Details
			</fluro-panel-title>
			<fluro-panel-body class="text-xs-center" v-if="connected">
				...
				<h4>Facebook has been successfully authorized</h4>
			</fluro-panel-body>
			<fluro-panel-body>
			...
			<fluro-panel-body v-if="!model._id"> Please save this integration to continue </fluro-panel-body>
		</fluro-panel>
	</div>
</template>
<script>
	...
	methods: {
		authorize(url) {
			...
			function redirectToURL(result) {
				var url = self.$fluro.api.generateEndpointURL(`/integrate/facebook/${self.model._id}/oauth`);
