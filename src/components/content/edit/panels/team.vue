<template>
  <flex-column>
    ...
    <tabset v-else :justified="true" :vertical="true">
      ...
      <tab
        :heading="`${model.assignments.length} Position${model.assignments.length == 1 ? '' : 's'}`"
      >
        <flex-column-body style="background: #fafafa;">
          <v-container class="grid-list-xl">
            <constrain md>
        ...
        <flex-column-body v-else style="background: #fafafa;">
          <v-container fluid>
            ...
            <template v-for="(eventtrack, index) in tracks">
              <team-track-teaser v-model="tracks[index]" :key="eventtrack._id" />
            </template>
          </v-container>
        </flex-column-body>
      </tab>
  ...
  computed: {
    ...
    fieldsOutput() {
      ...
      addField("visibleAssigned", {
        ...
        description:
          "Should assigned contacts know that they are in this group?",
        minimum: 0,
        maximum: 1,
        type: "boolean",
        expressions: {
      ...
      addField("provisionalRoles", {
        ...
        description:
          "Roles and permissions that should be granted to provisional members for this group/team",
        minimum: 0,
        maximum: 0,
        type: "reference",
        directive: "reference-select",
      ...
      addField("visibleMembers", {
        ...
        description:
          "Should members of this group know that they are in this group?",
        minimum: 0,
        maximum: 1,
        type: "boolean"
      });
