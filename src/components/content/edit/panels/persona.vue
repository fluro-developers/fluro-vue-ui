							...
							<constrain sm>
								...
								<template v-if="model.user">
									...
									<v-input class="no-flex">
										...
										<p class="help-block">
											To change {{ contextName }} password or email details click the button
											below, this will send an invitation to the user to create a new password and
								...
								<template v-else>
									...
									<fluro-panel v-if="showAdvanced">
										<fluro-panel-body>
											<h5 margin>
												{{ isManagedUser ? 'Whitelabel User Persona' : 'General Fluro User' }}
											</h5>
											<p v-if="isManagedUser">
												This user will exist only within the
												<strong>{{ user.account.title }}</strong> account and can only sign in
												to <strong>{{ user.account.title }}</strong> applications and websites.
												These kinds of user personas are typically used for white labelled apps
												or for young children that might not have an email address.
											</p>
											<p v-else>
												An email will be sent to {{ model.collectionEmail }} with an invitation
												to create a Fluro account (if they do not have one already) they will
												then be able to sign in to <em>{{ user.account.title }}</em> in addition
												to any other accounts they have been invited to on Fluro.
											</p>
											<template v-if="!model._id">
												<v-btn
													class="ma-0"
													tag="span"
													@click="createManaged = true"
													v-if="!isManagedUser"
												>
