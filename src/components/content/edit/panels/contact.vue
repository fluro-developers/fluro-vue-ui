								...
								<v-layout row wrap>
									<v-flex :class="{ xs12: $vuetify.breakpoint.xsOnly }">
										...
										<fluro-content-form-field
											...
											@input="update"
											:options="formOptions"
											:field="fieldHash.ethnicName"
											v-model="model"
											v-if="showEthnicName"
										></fluro-content-form-field>
										<div class="additions">
											<a class="hint" v-if="!showPreferredName" @click="show.preferredName = true"
												>Add Nick/Preferred Name</a
											>
											<a class="hint" v-if="!showEthnicName" @click="show.ethnicName = true"
												>Add Ethnic Name</a
											>
										</div>
									</v-flex>
									...
									<v-flex :class="{ xs12: $vuetify.breakpoint.xsOnly }">
										...
										<fluro-content-form-field
											...
											@input="update"
											:options="formOptions"
											:field="fieldHash.maidenName"
											v-model="model"
											v-if="showMaidenName"
										></fluro-content-form-field>
										<div class="additions">
											<a class="hint" v-if="!showMiddleName" @click="show.middleName = true"
												>Add Middle Name</a
											>
											<a class="hint" v-if="!showMaidenName" @click="show.maidenName = true"
												>Add Maiden Name</a
											>
										</div>
									</v-flex>
								...
								<wrapper xs>
									<v-input class="no-flex" v-if="isNotSubsplash">
										...
										<fluro-academic-select
											...
											:outline="showOutline"
											:options="formOptions"
											@calendar="updateAcademicCalendar"
											@grade="updateAcademicGrade"
											v-model="model"
										/>
									</v-input>
									<v-input class="no-flex">
										...
										<p class="help-block">
											Publicly displayed notes to be printed on {{ contextName }} nametag
										</p>
										<fluro-content-form-field
											:form-fields="formFields"
											:showLabel="false"
							...
							<constrain sm>
								<template v-if="context == 'edit'">
									...
									<v-layout row wrap>
										<v-flex xs12 sm6>
											<fluro-content-form-field
												:override-label="`${contextName} Household Role`"
												:form-fields="formFields"
												:outline="showOutline"
												@input="update"
												:options="formOptions"
											...
											<tab heading="Add to existing Household">
												<v-container>
													<v-input class="no-flex">
														<v-label>Family Household</v-label>
														<p class="help-block">
															Select {{ contextName }} primary residence
														</p>
														<fluro-content-form-field
															:form-fields="formFields"
															:showLabel="false"
								...
								<list-group v-if="personas.length">
									...
									>
										<template v-slot:right>
											<v-btn
												class="ma-0"
												@click.prevent.stop="$actions.open([item])"
												icon
												small
												flat
											>
						...
						<v-container grid-list-xl>
							<constrain sm>
									...
									<fluro-panel>
										<div class="toggle-items">
											<template v-if="emails.length">
												<fluro-toggle-item
													@click.native="toggleSubscription('mailout')"
													:inactive="unsubscribedMailout"
												>
													Allow Promotional Emails
												</fluro-toggle-item>
											</template>
											<template v-if="isNotSubsplash && phoneNumbers.length">
												<fluro-toggle-item
													@click.native="toggleSubscription('sms')"
													:inactive="unsubscribedSMS"
												>
								...
								<v-input class="no-flex" v-if="devices.length">
											...
											<template v-slot:default>
												<div>
													...
													<div class="sm text-muted">
														{{ device.metadata.platform }} {{ device.metadata.version }}
													</div>
												</div>
											</template>
											<template v-slot:right>
									...
									<p class="help-block">
										If you are experiencing problems with push notifications it can sometimes be
										helpful to reset all devices for a contact allowing them to reconnect next time
										they sign in to an application
									</p>
						...
						<v-container>
							<constrain sm>
								...
								<v-input class="no-flex">
									...
									<p class="help-block">
										Select where {{ contextName }} {{ sheet.title }} information should be stored
									</p>
									<fluro-realm-select
										@input="updateSheetRealms($event, sheet.definitionName)"
										v-model="sheet.dataModel.realms"
										type="contactdetail"
								...
								<fluro-content-form
									:options="formOptions"
									@input="updateSheet($event, sheet.definitionName)"
									v-model="sheet.dataModel.data"
									:fields="sheet.fields"
								/>
...
export default {
	...
	methods: {
		...
		ping(device) {
			...
			self.$fluro.api
				.get(`/push/ping/${device._id}`, { cache: false })
				.then(function (res) {
		...
		resetDevices() {
			...
			self.$fluro.api
				.get(`/push/clear/${self.itemID}`, { cache: false })
				.then(function (res) {
	...
	created() {
		...
		if (self.config && self.config.details && self.config.details.length) {
			...
			self.sheetDefinitions = _.reduce(
				self.config.details,
	...
	asyncComputed: {
		personas: {
			...
			get() {
				...
				return new Promise(function (resolve, reject) {
					self.$fluro.api
						.get(`/contact/${self.model._id}/personas`, { cache: false })
						.then(function (res) {
							resolve(res.data);
						})
						.catch(reject);
		...
		devices: {
			...
			get() {
				...
				return new Promise(function (resolve, reject) {
					self.$fluro.api
						.get(`/contact/${self.itemID}/devices`)
						.then(function (res) {
							resolve(
								_.map(res.data, function (device) {
									device.pinging = false;
									return device;
								})
							);
		...
		smsUnsubscribes: {
			...
			get() {
				...
				return new Promise(function (resolve, reject) {
					self.$fluro.api
						.get(`/contact/${self.itemID}/smsunsubscribes?${self.subscriptionCacheKey}`)
						.then(function (res) {
							resolve(res.data);
						})
						.catch(function (err) {
		...
		mailoutUnsubscribes: {
			...
			get() {
				...
				return new Promise(function (resolve, reject) {
					self.$fluro.api
						.get(`/contact/${self.itemID}/unsubscribes?${self.subscriptionCacheKey}`)
						.then(function (res) {
							resolve(res.data);
						})
						.catch(function (err) {
