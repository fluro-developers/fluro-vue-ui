				...
				<flex-column-body style="background: #fff">
					<v-container class="grid-list-xl">
						<constrain sm>
							...
							<fluro-content-form-field
								:override-label="
									definition && definition.definitionName && definition.definitionName == 'service'
				...
				<flex-column-body style="background: #fff">
					<v-container>
						<constrain sm>
							...
							<v-btn
								block
								@click="createRoster(rosterType)"
								:loading="rosterType.loading"
								class="btn-ghost"
								v-for="rosterType in rosterTypes"
								:key="rosterType.title"
								>Add {{ rosterType.title }}</v-btn
							>
							<p></p>
							...
							<div v-if="eventTrack">
								<event-reminder-manager
									...
									disabled
									:slots="rosterSlots"
									v-model="eventTrack.defaultReminders"
								/>
							</div>
					...
					<v-container>
						<constrain sm>
							...
							<fluro-content-form-field
								...
								@input="update"
								:options="options"
								:field="fieldHash.streamEnabled"
								v-model="model"
							/>
							<template v-if="model.streamEnabled">
								<fluro-content-form
									:options="options"
									v-model="model.streamData"
									:fields="streamFields"
								>
											...
											<v-layout row wrap>
												<v-flex xs12 sm6>
													<v-input class="no-flex">
														...
														<fluro-content-form-field
															...
															@input="update"
															:options="options"
															:field="fieldHash.streamStartOffset"
															v-model="model"
														/>
														<p class="lead">
															{{
																streamStartDate
																	| formatDate('h:mma dddd D MMM', model.timezone)
															}}
														</p>
													</v-input>
												</v-flex>
												<v-flex xs12 sm6>
													<v-input class="no-flex">
														...
														<p class="help-block">
															How many minutes after this event's end time should the
															stream be available?
														</p>
														<fluro-content-form-field
															:form-fields="formFields"
															@input="update"
															:options="options"
															:field="fieldHash.streamEndOffset"
															v-model="model"
														/>
														<p class="lead">
															{{
								...
								<fluro-content-form-field
									...
									@input="update"
									:options="options"
									:field="fieldHash.streamIntegrations"
									v-model="model"
								/>
								<fluro-content-form-field
									:form-fields="formFields"
									...
									<fluro-panel-body>
										<v-input class="no-flex" label="RTMP Upstream URL">
											<div
												...
												@click.stop.prevent="copyToClipboard('rtmpUrl')"
											>
												<v-layout align-center>
										...
										<v-input class="no-flex" label="RTMP Stream Key">
											<div
												v-tippy
												content="Click to copy to clipboard"
												@click.stop.prevent="copyToClipboard('streamKey')"
											>
												<v-layout align-center>
				...
				<flex-column-body style="background: #fff">
					<v-container>
						...
						<fluro-content-form-field
							...
							@input="update"
							:options="options"
							:field="fieldHash.publicTicketingEnabled"
							v-model="model"
						></fluro-content-form-field>
						...
						<fluro-panel>
							...
							<fluro-panel-body>
								<fluro-content-form
									:options="options"
									v-model="model.ticketCollectionData"
									:fields="ticketCollectionFields"
								>
									<template v-slot:form="{ formFields, fieldHash, model, update, options }">
										<v-container pa-0 grid-list-xl>
											<v-layout row wrap>
												<v-flex xs12 sm6>
													<v-input class="no-flex">
														...
														<p class="help-block">
															How many minutes earlier can tickets be collected
														</p>
														<fluro-content-form-field
															:form-fields="formFields"
															@input="update"
															:options="options"
															:field="fieldHash.ticketCollectionStartOffset"
															v-model="model"
														/>
												...
												<v-flex xs12 sm6>
													<v-input class="no-flex">
														...
														<p class="help-block">
															How many minutes after this event's end time can a ticket be
															collected?
														</p>
														<fluro-content-form-field
															:form-fields="formFields"
															@input="update"
															:options="options"
															:field="fieldHash.ticketCollectionEndOffset"
															v-model="model"
														/>
										...
										<v-layout row wrap>
											<v-flex xs12 sm6>
												<v-input class="no-flex">
													...
													<fluro-content-form-field
														...
														@input="update"
														:options="options"
														:field="fieldHash.checkinStartOffset"
														v-model="model"
													/>
											...
											<v-flex xs12 sm6>
												<v-input class="no-flex">
													...
													<p class="help-block">
														How many minutes after this event's end time can a user still
														check in?
													</p>
													<fluro-content-form-field
														:form-fields="formFields"
														@input="update"
														:options="options"
														:field="fieldHash.checkinEndOffset"
														v-model="model"
													/>
...
export default {
	...
	methods: {
		...
		createPlan() {
			...
			self.$fluro.global
				.select('plan', { title: 'Select a template', minimum: 1, maximum: 1 }, true)
				.then(function (plans) {
		...
		createRoster(rosterType) {
			...
			self.$fluro.api.post(`/content/${rosterType.definitionName}`, newRoster).then(
				function (res) {
					var roster = res.data;
	...
	computed: {
		...
		fieldsOutput() {
			...
			addField('socialImages', {
				...
				description:
					'Add images and artwork that can be shared from pulse and slipstream to promote this event',
				minimum: 0,
				maximum: 0,
				type: 'reference',
				directive: 'reference-select',
			...
			addField('forms', {
				...
				description: self.model.publicTicketingEnabled
					? 'Add your own custom form'
					: 'Link forms to this event.',
				minimum: 0,
				maximum: 0,
				type: 'reference',
				directive: 'reference-select',
			...
			addField('expectTeams', {
				title: 'Expect Groups',
				description:
					'Select groups to expect as attendees. This will expect all active members of these groups as attending at the time of event start',
				minimum: 0,
				maximum: 0,
				type: 'reference',
				directive: 'reference-select',
		...
		ticketCollectionEndDate() {
			var self = this;
			return self.$fluro.date.moment(self.model.endDate).add(self.ticketCollectionEndOffset, 'minutes').toDate();
		},
		checkinStartOffset() {
			var self = this;
		...
		checkinEndDate() {
			var self = this;
			return self.$fluro.date.moment(self.model.endDate).add(self.checkinEndOffset, 'minutes').toDate();
		},
