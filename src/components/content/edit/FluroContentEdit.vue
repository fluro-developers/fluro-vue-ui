					...
					<page-header :type="typeName">
						...
						<template v-slot:right>
							...
							<template v-if="showDeveloperDocs">
								<v-menu :nudge-width="250" offset-y left>
									...
									<v-card tile>
										<v-list dense>
											<v-list-tile
												href="https://fluro-developers.github.io/fluro-vue/"
												target="_blank"
											>
												<v-list-tile-title> Fluro Vue Documentation</v-list-tile-title>
											</v-list-tile>
											<v-list-tile
												href="https://fluro-developers.github.io/fluro/"
												target="_blank"
											>
											...
											<v-divider />
											<v-list-tile
												href="https://www.youtube.com/playlist?list=PLpML5U6BFHeFCFY23ed4ElSlXzydStw6l"
												target="_blank"
											>
							...
							<template v-if="showPadlock">
								<help
									title="Privacy Selector"
									body="Allows you to disable security for this content. By default websites apps and users need permission to access content in Fluro, clicking here can turn that off so that this content can be viewed on the web publicly without authentication"
								/>
							...
							<help
								...
								:body="`Everything in Fluro needs to be kept somewhere, choose one or more realms for this ${definitionTitle}. This will affect who has access to view and edit this ${definitionTitle}`"
							/>
							<!-- <pre>{{typeName}} - {{definitionName}}</pre> -->
							<fluro-realm-select
								ref="realmSelector"
								v-tippy
								:content="`Select where this ${definitionTitle} should be stored`"
								v-if="realmSelectorRelevant"
								v-model="model.realms"
								:type="typeName"
							...
							<fluro-tag-select class="mx-0 ml-2" v-if="tagsAvailable" v-model="model.tags">
								<help
									title="Tag Selector"
									:body="`Add tags to describe and make it easier to find this ${definitionTitle} when searching later`"
								/>
							</fluro-tag-select>
							<!-- <pre>{{model.tags}}</pre> -->
							<v-btn v-if="model._id" class="mx-0 ml-2" @click="$actions.open([model])">
								...
								<help
									title="More Actions"
									:body="`View more actions that are relevant to this ${definitionTitle}`"
								/>
							</v-btn>
						...
						<flex-column-footer class="border-top">
							<v-container fluid pa-1>
								<v-layout row align-center>
												...
												<template v-slot:activator="{ on }">
													...
													>
														<help
															...
															:body="`View system and database data for this  ${definitionTitle}`"
														/>
												...
												<v-container pa-2 class="white-background">
													<template v-if="model._id">
														<v-input class="no-flex" label="Fluro ID">
															<v-btn
																small
																@click="copyToClipboard(model._id)"
																block
																v-tippy
													...
													<template v-if="model._external" class="white-background">
														<v-input class="no-flex" label="External ID">
															<v-btn
																small
																@click="copyToClipboard(model._external)"
																block
																v-tippy
																content="Click to copy to clipboard"
																>{{ model._external }}</v-btn
															>
														</v-input>
													</template>
													<template>
												...
												<template v-slot:activator="{ on }">
													...
													>
														...
														<help
															...
															:body="`Set automatic publish and archiving dates`"
														/>
													</v-btn>
												</template>
									...
									<v-flex shrink>
										<template v-if="model._id">
											<em class="muted sm" style="margin-right: 10px"
												>Last updated {{ model.updated | timeago }}</em
											>
										</template>
										<help
											title="Status Selector"
											:body="`Update and change the status of this ${definitionTitle}`"
										/>
										<fluro-status-select
									...
									<v-flex shrink v-if="model._id">
										...
										<v-menu top :close-on-content-click="false" @click.native.stop offset-y>
											<template v-slot:activator="{ on }">
												<v-btn
													v-tippy
													content="History and Change log"
													class="my-0"
													small
													icon
													flat
												...
												>
													...
													<help
														title="Change log"
														:body="`View the system change log for this ${definitionTitle}. And rollback to previous versions.`"
													/>
												</v-btn>
											</template>
...
export default {
	...
	methods: {
		...
		submitUpdate() {
			...
			return new Promise(function (resolve, reject) {
				self.$fluro.api
					.put(`/content/${definedType}/${requestData._id}?replaceData=true`, requestData)
					.then(function (res) {
				...
				switch (context) {
					...
					default:
						...
						if (definedType == 'family') {
							//Create a new item
							self.$fluro.api.post(`/family`, requestData).then(resolve, reject);
						} else {
							//Create a new item
							self.$fluro.api.post(`/content/${definedType}`, requestData).then(resolve, reject);
						}
			...
			switch (self.typeName) {
				...
				case 'ticket':
					return self.model.event
						? `${self.model.event.title} ${self.$fluro.date.readableEventDate(self.model.event)}`
						: '';
					break;
				case 'plan':
