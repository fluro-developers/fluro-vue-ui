    ...
    <fluro-panel margin>
        <fluro-panel-body>
            <v-layout align-center>
                <v-flex>
                    ...
                    <p class="help-block">Expressions and extra options for managing multiple gateways. Ensure that at least one gateway is enabled for form payments to be enabled</p>
                    <!-- </v-input> -->
                </v-flex>
                <v-flex shrink>
        ...
        <template v-if="expanded">
            <fluro-panel-body class="border-top" v-for="gateway in gateways">
                <v-text-field :label="`${gateway.title} - Title`" persistent-hint hint="Update the gateway title displayed to the user" :placeholder="gateway.title" v-model="model[gateway._id].title" />
                <!-- <div class="expression-group" :class="{active:model[gateway._id].expression}"> -->
                <v-input label="Enable Expression" :hint="`Enable '${gateway.title}' if expression returns true. Leave blank to always enable`" :persistent-hint="true" class="no-flex">
                    <v-layout>
                        <v-flex>
                            <fluro-expression-editor v-model="model[gateway._id].expression" />
                        </v-flex>
                        <v-flex shrink>
                            <v-menu :left="true" v-model="popup[gateway._id]" :top="true" :close-on-content-click="false" transition="slide-y-transition" offset-y>
                                ...
                                <div>
                                    <expression-field-select :conditional="true" :context="paymentGatewayContextField" @click="injectExpression($event, gateway._id)" v-model="form.fields" />
                                </div>
                            </v-menu>
                        </v-flex>
                    </v-layout>
