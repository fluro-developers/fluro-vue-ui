		...
		<v-menu left offset-x :open-on-hover="true">
			...
			<div class="suggestion-roster-summary">
				...
				<div class="suggestion-roster-summary-group" v-if="assignmentsExplanation">
					<h6><fluro-icon library="fas" class="icon" type="assignment" />Already Declined</h6>
					<div v-html="assignmentsExplanation" />
				</div>
				<div class="suggestion-roster-summary-group" v-if="teams">
					<h6>
						<fluro-icon library="fas" class="icon" type="team" /> Is '{{ assignmentSlot.title }}' position
						in
					</h6>
					<div v-html="teams" />
				</div>
				<div class="suggestion-roster-summary-group" v-if="capabilities">
					<h6><fluro-icon library="fas" class="icon" type="capability" />Capabilities</h6>
					<div v-html="capabilities" />
				</div>
				<div class="suggestion-roster-summary-group" v-if="conflicts.length">
					<h6><fluro-icon library="fas" class="icon" icon="exclamation-circle" /> Conflicts</h6>
					<div v-for="assignment in conflicts">
						<strong>{{ assignment.title }}</strong> at
						<strong
							>{{ assignment.event.startDate | formatDate('h:mma') }} - '{{
				...
				<div class="suggestion-roster-summary-group" v-if="prior">
					<h6><fluro-icon library="fas" class="icon" type="angle-left" /> Prior Assignment</h6>
					<div v-html="prior"></div>
				</div>
				<div class="suggestion-roster-summary-group" v-if="next">
					<h6><fluro-icon library="fas" class="icon" type="angle-right" /> Next Assignment</h6>
					<div v-html="next"></div>
				</div>
				<div class="suggestion-roster-summary-group" v-if="lacks.length">
					<h6><fluro-icon library="fas" class="icon" type="frown" /> Lacks required capability</h6>
					<div>
						<Strong>{{ lacks }}</Strong>
					</div>
				</div>
				<div class="suggestion-roster-summary-group" v-if="periods.length">
					<h6><fluro-icon library="fas" class="icon" type="times-circle" /> Unavailable</h6>
...
export default {
	...
	computed: {
		...
		conflictExplanation() {
			...
			return `<strong>${_.values(reasonsMap.descriptions).join(', ')}</strong> (${_.values(
				reasonsMap.positions
			).join(', ')})`;
		},
		...
		capabilities() {
			return this.model.capabilities && this.model.capabilities.length
				? _.map(this.model.capabilities, 'title').join(', ')
				: null;
		},
		endDate() {
