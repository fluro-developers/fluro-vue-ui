	...
	<div v-if="workingModel">
		<fluro-panel>
			<tabset :justified="true" :vertical="false">
				<tab
					:heading="getTitle(def)"
					:muted="muted(workingModel[def.definitionName])"
					:icon="iconObject(workingModel[def.definitionName])"
					v-for="(def, ind) in rosterOptions"
				>
					<fluro-panel-body>
						<default-roster-component
							@input="updateModel"
							:config="config"
							:data="def"
							v-model="workingModel[def.definitionName]"
						/>
					</fluro-panel-body>
				</tab>
			</tabset>
