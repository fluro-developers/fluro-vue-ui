					...
					<v-layout align-center>
						...
						<v-flex>
							<v-layout v-if="column.manual || !queryID" align-center>
								...
								<v-flex shrink>
									<v-btn
										@click="setManual(index, false)"
										v-tippy
										content="Select from available fields"
										class="ma-0"
										small
										icon
									>
							...
							<v-layout v-else align-center>
								...
								<v-flex shrink>
									<v-btn
										@click="setManual(index, true)"
										v-tippy
										content="Manually enter a key"
										class="ma-0"
										small
										icon
									>
...
export default {
	...
	methods: {
		add() {
			...
			copy.description =
				'The type of data that will be rendered in this column, this affects how the data will be displayed and sorted.';
			fields.push(copy);

			self.$fluro.prompt(fields).then(function (entry) {
				var column = entry;
		...
		keySample: {
			...
			get() {
				...
				return new Promise(function (resolve, reject) {
					...
					self.$fluro.api.get(`/content/_query/${self.queryID}`).then(
						function (res) {
							resolve(res.data);
							self.loadingKeys = false;
						},
				...
				.reduce(function (set, entry) {
					...
					_.each(flattened, function (value, key) {
						...
						if (!existingEntry) {
							...
							if (_.isString(matchObject)) {
								var momentDate = self.$fluro.date.moment(
									matchObject,
									self.$fluro.date.moment.ISO_8601,
									true
								);
								var isValid = momentDate.isValid();
								if (isValid) {
				...
				.map(function (value, key) {
					return {
						...
						description:
							value.type == 'array'
								? '(Multiple Values)'
								: `Values include: ${value.examples.slice(0, 3).join(', ')}`,
					};
				})
