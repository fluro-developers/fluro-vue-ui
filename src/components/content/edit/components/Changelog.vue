								...
								>
									<v-layout align-center>
										<v-flex shrink class="pr-1">
											<fluro-avatar
												v-if="entry.managedUser"
												:id="entry.managedUser"
												type="persona"
											/>
										...
										<v-flex>
											...
											<div class="sm muted">
												{{ entry.key }} - {{ entry.managedUser ? entry.managedUser.title : '' }}
											</div>
										</v-flex>
...
export default {
	...
	methods: {
		promptOptions(entry) {
			...
			return self.$fluro.options(options, `Log options`).then(function (result) {
				switch (result.value) {
					case 'log':
					...
					case 'rollback':
						self.$fluro
							.confirm(
								'Confirm Rollback',
								`Are you sure you want to rollback ${self.item.title} to ${self.$fluro.date.formatDate(
									entry.created,
									'h:mma dddd D MMM YYYY'
								)}?`,
								{
									confirmColor: 'error',
							...
							.then(function () {
								...
								self.$fluro.api.post(`/content/rollback/${entry._id}`).then(function (result) {
