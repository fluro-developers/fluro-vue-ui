						...
						<list-group class="full-width">
							<list-group-item v-for="(slot, ind) in data.data.slots" class="full-width">
								<default-roster-slot
									@input="update"
									:config="config"
									:slotData="data.data.slots[ind]"
									v-model="model.slots[ind]"
								/>
							</list-group-item>
						</list-group>
					</v-flex>
...
export default {
	...
	computed: {
		customFields() {
			...
			addField('create', {
				title: `Create ${_.get(self.data, 'title')} ${
					_.get(self.data, 'title').toLowerCase().includes('roster') ? '' : 'Roster'
				}`,
				description: `Create a ${_.get(self.data, 'title')} ${
					_.get(self.data, 'title').toLowerCase().includes('roster') ? '' : 'Roster'
				} for events created in this track if one does not already exist`,
				minimum: 0,
				maximum: 1,
				type: 'boolean',
			});
