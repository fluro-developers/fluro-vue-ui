		...
		<flex-column-header>
			<v-container class="border-bottom">
				<v-layout align-center>
					<v-flex>
						<h4>
							<strong v-if="slot">{{ slot.title }}</strong>
						</h4>
					</v-flex>
				</v-layout>
				<h6 v-if="event">
					{{ event.title }} <span class="muted">{{ event.definition || event._type | definitionTitle }}</span>
				</h6>
				<div v-if="event">
					{{ event | readableEventDate }}
					<span class="sm muted">(Starts {{ event.startDate | timeago }})</span>
				</div>
			</v-container>
		</flex-column-header>
		<flex-column>
									...
									<template v-else>
										...
										<list-group>
											<roster-slot-suggestion-list-item
												...
												:manual="true"
												@click.native="selectSuggestion(searchResults[index])"
								...
								<v-container px-0 pt-1>
									...
									<div v-else>
										<list-group v-if="suggestedContacts.length">
											<roster-slot-suggestion-list-item
												v-for="(contact, index) in suggestedContacts"
												@click.native="selectSuggestion(suggestedContacts[index])"
												v-model="suggestedContacts[index]"
												:assignmentSlot="slot"
												:event="event"
											/>
					...
					<flex-column-body>
						<v-container>
							<v-container pa-0 pb-2 v-if="conflicts.length">
								...
								<list-group>
									<roster-slot-suggestion-list-item
										v-for="(contact, index) in conflicts"
										@click.native="selectSuggestion(conflicts[index])"
										v-model="conflicts[index]"
										:assignmentSlot="slot"
										:event="event"
									/>
								</list-group>
							...
							<v-container pa-0 pb-2 v-if="unavailable.length">
								...
								<list-group>
									<roster-slot-suggestion-list-item
										v-for="(contact, index) in unavailable"
										@click.native="selectSuggestion(unavailable[index])"
										v-model="unavailable[index]"
										:assignmentSlot="slot"
										:event="event"
									/>
								</list-group>
							...
							<v-container pa-0 pb-2 v-if="unqualified.length">
								...
								<list-group>
									<roster-slot-suggestion-list-item
										v-for="(contact, index) in unqualified"
										@click.native="selectSuggestion(unqualified[index])"
										v-model="unqualified[index]"
										:assignmentSlot="slot"
										:event="event"
									/>
								</list-group>
...
export default {
	...
	methods: {
		...
		eventRealmMap() {
			...
			return _.reduce(
				self.relevantRealmIDs,
				function (set, realmID) {
					set[realmID] = realmID;
					return set;
				},
				{}
		...
		searchFor: _.debounce(function (string) {
			...
			self.$fluro.api
				.get(`/content/contact/search/${string}`, {
					params: {
						limit: 10,
					},
					cache: false,
			...
			return new Promise(function (resolve, reject) {
				...
				function assignmentCreated(res) {
					...
					matchingAssignment.contactName =
						...
						matchingAssignment.contact.firstName + ' ' + matchingAssignment.contact.lastName;
						...
						var array = [
							{
								...
								description:
									(contact.firstName || 'this person') + ' can still fulfill this assignment',
								force: true,
							},
						];
		...
		sendAssignment(assignment) {
			...
			return self.$fluro.api
				.put(`/content/assignment/${assignment._id}`, {
		...
		removeAssignment(assignment) {
			...
			return self.$fluro
				.confirm('Confirm Deletion', `Are you sure you want to delete this assignment`, {
					confirmColor: 'error',
					confirmText: 'Confirm Delete',
				})
				.then(function (res) {
					return self.$fluro.api
						.delete(`/content/assignment/${assignment._id}`)
						.then(function (res) {
							removeComplete(res.data);
						})
						.catch(function (err) {
