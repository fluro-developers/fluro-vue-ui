			...
			<flex-column>
				...
				<flex-column-body>
					<v-container>
						<list-group>
							<list-group-item
								v-for="(slot, slotIndex) in model.slots"
								:key="`${slot.title}`"
								class="pa-3"
							>
								<roster-slot-manager
									v-model="model.slots[slotIndex]"
									@selectcell="selectSlotCell(slot, slotIndex)"
									:selected="isSelectedSlot(model, slot.title)"
									:definition="definition"
									:roster="model"
									@click.native="selectSlotCell(slot, slotIndex)"
								/>
							</list-group-item>
						</list-group>
					</v-container>
	...
	methods: {
		...
		assignTeam(roster) {
			...
			return new Promise(function (resolve, reject) {
				...
				self.$fluro.global
					.select('team', { allDefinitions: true, maximum: 1, minimum: 1, filter: initialFilter })
					.then(function (teams) {
		...
		isSelectedSlot(roster, slotName) {
			...
			return (
				self.selected.roster == roster &&
				String(slotName).toLowerCase() == String(self.selected.slot.title).toLowerCase()
			);
		},
