    ...
    <flex-row>
      ...
      <flex-column-body class="help">
        <v-container>
          ...
          <p
            class="help-block"
          >You can use the variables described below within your value and condition fields.</p>
          <v-btn
            small
...
export default {
  ...
  computed: {
    ...
    availableFields() {
      ...
      function extractFieldsFromDefinitionFields(
        fields,
        indexIterator,
        includeExamples
      ) {
        ...
        function formatField(field, index) {
          ...
          if (field.options && field.options.length) {
            ...
            firstTen = _.map(field.allowedValues.slice(0, cutOff), function(
              entry
            ) {
              if (entry) {
                return {
            ...
            firstTen = _.map(field.defaultValues.slice(0, cutOff), function(
              entry
            ) {
              if (entry) {
                return {
          ...
          if (field.childPath) {
            ...
            if (field.isArray) {
              ...
              _.each(firstTen, function(option) {
                examples.push({
                  path:
                    "data." + fieldPath + ".includes('" + option.value + "')",
                  description:
                    "Returns true if  '" +
                    option.name +
              ...
              _.each(firstTen, function(option) {
                examples.push({
                  ...
                  description:
                    "Returns true if  '" + option.name + "' has been selected"
                });
              });
              ...
              switch (field.type) {
                case "boolean":
                  examples.push({
                    ...
                    description:
                      "If '" + field.title + "' is equal to " + exampleValue
                  });

                  examples.push({
                    path: "data." + fieldPath + " != " + exampleValue,
                    description:
                      "If '" + field.title + "' is not equal to " + exampleValue
                  });
                  break;
                ...
                case "string":
                  if (!firstTen.length) {
                    ...
                    if (
                      field.directive == "wysiwyg" ||
                      field.directive == "textarea"
                    ) {
                      examples.push({
                ...
                case "float":
                  ...
                  examples.push({
                    path: "data." + fieldPath + " > " + exampleValue,
                    description:
                      "If '" + field.title + "' is greater than " + exampleValue
                  });
                  ...
                  examples.push({
                    path: "data." + fieldPath + " < " + exampleValue,
                    description:
                      "If '" + field.title + "' is less than " + exampleValue
                  });
