            ...
            <template v-slot:form="{formFields, fieldHash, model, update, options}">
                <v-layout column>
                    ...
                    <v-flex class="less-padding">
                        <fluro-content-form-field :form-fields="formFields" @input="update" :options="options" :field="fieldHash.householdRole" v-model="model"></fluro-content-form-field>
                    </v-flex>
                    <v-flex class="less-padding" v-if="contactDefinitions && contactDefinitions.length">
                        <fluro-content-form-field :form-fields="formFields" @input="update" :options="options" :field="fieldHash.definition" v-model="model"></fluro-content-form-field>
                    </v-flex>
                </v-layout>
                <v-layout align-center>
                    <v-btn class="ma-1" v-tippy content="Edit" @click="editContact()">
                        <fluro-icon icon="pencil" />
                    </v-btn>
                    <v-btn class="ma-1" v-tippy content="View" @click="viewContact()">
                        <fluro-icon icon="user" />
                    </v-btn>
                    <v-btn class="ma-1" v-tippy content="Remove" @click="removeContact()">
...
export default {
    ...
    methods: {
        removeContact() {
            ...
            self.$fluro.confirm(`Are you sure you want to remove ${contactName} from this household?`, null, options)
                .then(function(res) {
            ...
            // self.$fluro.confirm(self.model, true).then(function(res) {
