	...
	<div>
		<list-group>
			...
			>
				<list-group-item
					:key="index"
					v-for="(assignment, index) in model"
					class="pa-0"
					:class="currentlyExpanded[index] ? 'hover-grey' : ''"
					style="width: 100%"
				>
					<v-layout align-center ma-0 py-0 px-2>
						<v-flex xs3>
							...
							<div v-if="assignment.reporter" class="sm text-muted">
								<fluro-icon icon="chart" /> Reporter
							</div>
							<div v-if="assignment.roles && assignment.roles.length" class="sm text-muted">
								{{ assignment.roles | comma('title') }}
							</div>
						</v-flex>
						<v-flex xs5>
							<v-chip small :key="contact._id" v-for="contact in assignment.contacts">{{
								contact.title
							}}</v-chip>
						</v-flex>
						<v-flex xs2>
					...
					<tabset v-if="currentlyExpanded[index]" style="width: 100%">
						...
						<tab heading="Advanced">
							<v-layout wrap pa-4>
								<v-flex xs12 sm6>
									<v-checkbox
										v-model="model[index].shareDetails"
										:label="`Allow ${model[index].title} to view phone numbers and emails of other group members`"
										hint="Should people in this position be able to see the email and phone number of other members in this group/team?"
										persistent-hint
									></v-checkbox>
								</v-flex>
								<v-flex xs12 sm6>
									<v-checkbox
										v-model="model[index].visible"
										label="Visible"
										hint="Should this group be visible to people in this slot?"
										persistent-hint
									></v-checkbox>
								</v-flex>
								<v-flex xs12 sm6>
									<v-checkbox
										v-model="model[index].exclude"
										:label="`Exclude ${model[index].title} from membership`"
										hint="If checked, these contacts will not be considered 'members' of the group. They will be excluded from reports and other statistics. Typically this would be used for an oversight or an administrative only position."
										persistent-hint
									></v-checkbox>
								</v-flex>
							</v-layout>
	...
	methods: {
		addSlot() {
			...
			fields.push({
				...
				key: 'roles',
				description:
					'Select access roles that will be granted within the context of this group to anyone added into this position',
				minimum: 0,
				maximum: 0,
				type: 'reference',
				params: {
	...
	computed: {
		...
		editFields() {
			...
			addField('roles', {
				...
				description:
					'Roles and permissions that should be assigned to people in this position for this group/team',
				minimum: 0,
				maximum: 0,
				type: 'reference',
				directive: 'reference-select',
