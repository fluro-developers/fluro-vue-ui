		...
		<table>
			...
			<draggable class="options-manager-items" @sort="sorted" element="tbody" handle=".handle" v-model="model">
				<tr class="option" :key="option.name" v-for="(option, index) in model">
					...
					<td>
						<input @change="commit($event, model[index], 'name')" :value="model[index].name" placeholder="Name" />
					</td>
					<td>
						<input @change="commit($event, model[index], 'value')" :value="model[index].value" placeholder="Value" />
					</td>
					<td>
