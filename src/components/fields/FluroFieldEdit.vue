<template>
	<flex-column>
		...
		<tabset v-model="tabIndex" v-if="model.type == 'group'">
			<tab heading="Group Settings" index="options">
				<flex-column-body>
					<v-container>
						...
						<fluro-content-form-field
							@input="resetRequired(fields.asObject)"
							:field="fields.asObject"
							v-model="model"
						/>
						...
						<v-layout v-if="model.asObject">
							...
							<v-flex xs6>
								<fluro-content-form-field
									@input="resetRequired(fields.groupMaximum)"
									:field="fields.groupMaximum"
									v-model="model"
								/>
							</v-flex>
							<v-spacer />
							<v-flex xs6>
								<fluro-content-form-field
									@input="resetRequired(fields.askCount)"
									:field="fields.askCount"
									v-model="model"
								/>
							</v-flex>
						</v-layout>
				...
				<flex-column-body>
					<v-container class="border-bottom" style="background: #fff">
						...
						<v-btn
							class="ml-0"
							small
							href="https://support.fluro.io/kb/advanced-form-expressions-guide"
							target="_blank"
						>
						...
						<fluro-panel>
							...
							<fluro-panel-body class="border-top">
								<v-layout>
									...
									<v-flex xs9 class="sm muted"
										>The immediate model this field is attached to. Useful for relative paths to
										sibling fields</v-flex
									>
								</v-layout>
							</fluro-panel-body>
							<fluro-panel-body class="border-top">
								<v-layout>
									...
									<v-flex xs9 class="sm muted"
										>The root level model for the entire form, useful for absolute paths to other
										fields.</v-flex
									>
								</v-layout>
							</fluro-panel-body>
					...
					<v-container>
						...
						<div class="expression-group" :class="{ active: model.expressions.show }">
							<v-input
								label="Show group if"
								hint="Show this group only if this expression returns true "
										...
										>
											...
											<div>
												<expression-field-select
													:conditional="true"
													:context="model"
													@click="injectExpression($event, 'show')"
													v-model="expressionFields"
												/>
											</div>
										</v-menu>
									</v-flex>
						...
						<div class="expression-group" :class="{ active: model.hideExpression }">
							<v-input
								label="Hide group if"
								hint="Hide this group if this expression returns true "
								:persistent-hint="true"
								class="no-flex"
							>
								<v-layout>
										...
										>
											...
											<div>
												<expression-field-select
													:conditional="true"
													:context="model"
													@click="injectExpression($event, 'hideExpression')"
													v-model="expressionFields"
												/>
											</div>
										</v-menu>
			...
			<tab heading="Field Settings" index="options">
				<flex-column-body ref="scrollbox">
					<v-container>
						...
						<template
							v-if="model.directive == 'app-field-key-select' || model.directive == 'app-field-select'"
						>
							...
							<v-layout>
								...
								<v-flex xs6>
									<fluro-content-form-field
										...
										@input="resetRequired(fields.maximum)"
										v-model="model"
									/>
								</v-flex>
						...
						<template v-else>
							...
							<template v-if="model.directive != 'embedded'">
								<template v-if="model.type == 'reference'">
									...
									<fluro-content-form-field
										@input="resetRequired(fields.defaultReferences)"
										:field="fields.defaultReferences"
										v-model="model"
									/>
								</template>
								<template v-else>
									<template v-if="model.directive == 'wysiwyg'">
										<fluro-content-form-field
											@input="resetRequired(fields.wysiwygDefaultValues)"
											:field="fields.wysiwygDefaultValues"
											v-model="model"
										/>
									</template>
									<template v-else-if="model.directive == 'code'">
										<fluro-content-form-field
											@input="resetRequired(fields.codeDefaultValues)"
											:field="fields.codeDefaultValues"
											v-model="model"
										/>
									</template>
									<template v-else-if="model.directive == 'app-filter-select'">
										<fluro-content-form-field
											@input="resetRequired(fields.defaultValues)"
											:field="fields.filterDefaultValues"
											v-model="model"
										/>
									</template>
									<template v-else>
							...
							<fluro-panel v-if="model.directive == 'embedded' && restrictType == 'contact'">
								...
								<fluro-panel-body>
									...
									<v-input class="no-flex">
										...
										<v-btn class="ma-0" :loading="loadingFields" @click="selectDetailSheetFields"
											>Select Fields</v-btn
										>
									</v-input>
									<fluro-content-form-field :field="fields.targetStatus" v-model="model.params" />
									<fluro-content-form-field
										:field="fields.targetHouseholdRole"
										v-model="model.params"
									/>
									...
									<fluro-content-form v-model="model.params.ticketing" :fields="ticketingFields">
										<template v-slot:form="{ formFields, fieldHash, model, update, options }">
											...
											<template v-if="ticketingEnabled">
												<component
													ref="ticketingManager"
													:is="ticketingManager"
													:field="field"
													v-model="model.events"
													v-if="ticketingManager"
												/>
											</template>
							...
							<fluro-panel v-if="model.type == 'reference' && isPro">
								<fluro-panel-body>
									...
									<fluro-content-form-field
										@input="resetRequired(fields.defaultValues)"
										:field="fields.populationDetails"
										v-model="model.params"
									/>
								</fluro-panel-body>
							</fluro-panel>
				...
				<flex-column-body>
					...
					<v-container class="border-bottom" style="background: #fff">
						...
						<v-layout>
							...
							<v-flex xs9 class="sm muted"
								>The root level model for the entire form, useful for absolute paths.</v-flex
							>
						</v-layout>
						<v-layout>
							<v-flex xs3>
							...
							<v-flex xs9 class="sm muted"
								>The immediate model this field is attached to. Useful for relative paths</v-flex
							>
						</v-layout>
					</v-container>
					<v-container>
						<div class="expression-group" :class="{ active: model.expressions.show }">
							<v-input
								...
								hint="Show this field only if this expression returns true "
								:persistent-hint="true"
								class="no-flex"
							>
								<v-layout>
						...
						<div class="expression-group" :class="{ active: model.expressions.hide }">
							<v-input
								...
								hint="Hide this field if this expression returns true "
								:persistent-hint="true"
								class="no-flex"
							>
								<v-layout>
										...
										>
											...
											<div>
												<expression-field-select
													:conditional="true"
													:context="model"
													@click="injectExpression($event, 'hide')"
													v-model="expressionFields"
												/>
											</div>
										</v-menu>
						...
						<div
							...
							:class="{ active: model.expressions.defaultValue }"
							v-if="simpleExpressionEnabled"
						>
							<v-input
								label="Set default value to"
								hint="Set the default value, (the value before the user changes it) of this field depending on the input of another. "
								:persistent-hint="true"
								class="no-flex"
							>
								<v-layout>
									...
									<v-flex shrink>
										...
										>
											...
											<div>
												<expression-field-select
													:context="model"
													@click="injectExpression($event, 'defaultValue')"
													v-model="expressionFields"
												/>
											</div>
										</v-menu>
						...
						<div
							class="expression-group"
							:class="{ active: model.expressions.value }"
							v-if="simpleExpressionEnabled"
						>
							<v-input
								label="Set value to"
								hint="Set the value of this field depending on the input of another"
								:persistent-hint="true"
								class="no-flex"
							>
								<v-layout>
									...
									<v-flex shrink>
										...
										>
											...
											<div>
												<expression-field-select
													:context="model"
													@click="injectExpression($event, 'value')"
													v-model="expressionFields"
												/>
											</div>
										</v-menu>
						...
						<div
							class="expression-group"
							:class="{ active: model.expressions.required }"
							v-if="simpleExpressionEnabled"
						>
							<v-input
								label="Require this field if"
								hint="Require input for this field if it's visible and this expression returns true "
								:persistent-hint="true"
								class="no-flex"
							>
								<v-layout>
										...
										>
											...
											<div>
												<expression-field-select
													:conditional="true"
													:context="model"
													@click="injectExpression($event, 'required')"
													v-model="expressionFields"
												/>
											</div>
										</v-menu>
...
export default {
	...
	methods: {
		...
		selectDetailSheetFields() {
			...
			return self.$fluro.api
				.get('/defined/types/contactdetail')
				.then(function (res) {
					self.loadingFields = false;

					self.$fluro
						.options(res.data, 'Select detail sheet')
						.then(function (definition) {
							...
							var sheetBlock = _.find(detailsBlock.fields, {
							...
							_.each(definition.fields, function (field) {
								// guid:self.$fluro.utils.guid(),
								var existingField = _.find(dataBlock.fields, {
									key: field.key,
								});
								if (!existingField) {
	...
	computed: {
		...
		ticketingEnabled() {
			return (
				this.model && this.model.params && this.model.params.ticketing && this.model.params.ticketing.enabled
			);
		},
		ticketingManager() {
		...
		fields() {
			...
			addField('population', {
				...
				description: `When this data is retrieved select which fields should be included, Eg. If you a referencing an article you may want to retrieve the 'body' key in addition to the basics like 'title', '_id', 'created'`,
				minimum: 0,
				maximum: 1,
				type: 'string',
				directive: 'select',
				options: [
					{
						title: 'Normal (Just load the basics)',
						value: '',
					},
					{
						title: 'Full (Entire object)',
						value: 'full',
					},
					{
						title: 'Additional (Includes basic fields)',
						value: 'additional',
					},

					{
						title: 'Custom (Load specific fields)',
						value: 'custom',
					},
				],
			});

			addField('populationDetails', {
				title: 'Fields to select',
				description: `Seperate each field key with a space`,
				minimum: 0,
				maximum: 1,
				type: 'string',
				expressions: {
					show: `model.population == 'additional' || model.population == 'custom'`,
				},
			});

			// <div class="form-group" ng-if="model.params.population == 'additional' || model.params.population == 'custom'">
			//         <label>Populate Details</label>
			//         <p class="help-block">Seperate each field with a space</p>
			//         <input class="form-control" ng-model="model.params.populationDetails" />
			//     </div>
			...
			if (self.isPro) {
				addField('errorMessage', {
					...
					description:
						'Customise the message to display to the user if they enter invalid answers for this field',
					minimum: 0,
					maximum: 1,
					type: 'string',
				});
			...
			addField('maximum', {
				title: 'Max Answers',
				description:
					'Maximum amount of answers that can be provided for this field (usually 1 answer is necessary)',
				minimum: 0,
				maximum: 1,
				type: 'integer',
				params: {
			...
			addField('minimum', {
				...
				description:
					'Minimum amount of answers that can be provided for this field (Setting this to a number higher than 0 will make this field required)',
				minimum: 0,
				maximum: 1,
				type: 'integer',
				params: {
			...
			addField('minValue', {
				...
				description:
					self.model.directive == 'currency'
						? 'Minimum amount in cents that can be input'
						: 'Minimum amount that can be input',
				minimum: 0,
				maximum: 1,
			...
			addField('maxValue', {
				...
				description:
					self.model.directive == 'currency'
						? 'Maximum amount in cents that can be input'
						: 'Maximum amount that can be input',
				minimum: 0,
				maximum: 1,
				type: self.model.type,
				directive: self.model.directive || null,
			...
			addField('placeholder', {
				title: 'Placeholder',
				description:
					'Faded text that can be displayed inside the field to give a hint as to what the user should type',
				minimum: 0,
				maximum: 1,
				type: 'string',
			});
			...
			addField('showAllValues', {
				...
				description:
					'Tick this to allow the user to filter on all possible values that could be selected, if left unchecked then values will be cropped to only show from current results available.',
				minimum: 0,
				maximum: 1,
				type: 'boolean',
				//directive: "select",
			...
			addField('storeCopy', {
				...
				description:
					'Select whether to store a copy of the terms being agreed to as they are presented upon form submission.',
				minimum: 0,
				maximum: 1,
				type: 'boolean',
			});
