			...
			<template v-if="comparator && inputType != 'none'">
				...
				<v-flex xs12 sm5 v-else-if="inputType == 'array' && dataType != 'date'">
					<template v-if="simpleKeyIsRealms">
						<fluro-realm-select
							action="view any"
							:filterDiscriminator="discriminator || discriminatorType || discriminatorDefinition"
							block
							small
				...
				<v-flex xs12 sm5 v-else>
									...
									<template v-slot:activator="{ on }">
										...
										<template v-else>
											<v-text-field
												...
												single-line
												label="Date"
												:placeholder="model.computedValue"
												v-on="on"
											/>
										</template>
									</template>
					...
					<v-text-field
						v-else-if="dataType == 'number' || dataType == 'float' || dataType == 'decimal'"
						class="small-input"
						single-line
						v-model="model.value"
					/>
					...
					<v-layout :key="criteriaRow.guid" v-for="(criteriaRow, index) in model.criteria">
						<v-flex>
							<filter-condition-row
								...
								:rows="rows"
								:fields="selector.subfields"
								:mini="mini"
								v-model="model.criteria[index]"
							/>
						</v-flex>
						<v-flex shrink>
...
export default {
	...
	beforeCreate() {
		...
		Promise.all([
			DynamicImportService.load('./FilterConditionGroup.vue', function () {
				return import('./FilterConditionGroup.vue');
			}),
			DynamicImportService.load('./FilterConditionRow.vue', function () {
				return import('./FilterConditionRow.vue');
			}),
			DynamicImportService.load('../contentselect/FluroContentSelectButton.vue', function () {
				return import('../contentselect/FluroContentSelectButton.vue');
			}),
		...
		retrieveValues() {
			...
			if (!valueCache) {
				...
				if (rawRowsAlreadyHaveKey) {
					...
					valueCache = valueStorageCache[self.rowChangeString] = self.$fluro.content.values(
						subSetIDs,
						key,
						options
					);
