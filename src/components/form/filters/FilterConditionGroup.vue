	...
	<div class="filter-group-outer" :class="{ mini: mini, large: !mini }">
		<v-container ref="element" :class="[{ 'pa-0': mini }, model.operator]">
			...
			<v-layout row :key="group.guid" v-for="(group, g) in model.filters">
				<div
					class="operator-column offset"
					:class="model.operator"
					v-if="model.filters.length > 1"
					d-flex
					align-center
				>
					<span class="operator" :class="[{ additional: g }, model.operator]">{{
						operatingWord(model, g)
					}}</span>
				</div>
				<v-flex style="overflow: hidden">
					<v-container
						:class="[{ 'pa-2': mini }, { 'py-2 px-0': !mini }, group.operator]"
						class="additional filter-group"
					>
							...
							<div v-for="(filter, index) in group.filters" :key="filter.guid">
								<v-layout>
									<div
										...
										align-center
									>
										<span class="operator" :class="[{ additional: index }, group.operator]">{{
											operatingWord(group, index)
										}}</span>
									</div>
									<v-flex style="overflow: hidden">
