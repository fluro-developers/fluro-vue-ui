						...
						<v-layout align-center>
							...
							<v-flex shrink>
								...
								<v-btn
									...
									class="ma-0 ml-1"
									small
									@click="$actions.open(contextSelection.selection)"
								>
									More Actions
								</v-btn>
							</v-flex>
					...
					action(row) {
						return new Promise(function (resolve, reject) {
							if (self.confirmRemove && self.$fluro.confirm) {
								self.$fluro
									.confirm('Remove from list', `Deselect ${row.title}?`)
									.then(deselect)
									.catch(reject);
							} else {
								deselect();
		...
		showModal() {
			...
			if (!self.type || self.type == 'node') {
				self.$fluro.types.basicTypes().then(function (types) {
					...
					self.$fluro
						.options(answers, 'Select a type', 'Select what kind of thing you want to reference')
