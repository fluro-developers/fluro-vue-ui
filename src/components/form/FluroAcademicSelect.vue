    ...
    <div class="fluro-academic-select">
        ...
        <v-input class="no-flex" :label="label" :required="required" :error-messages="errorMessages" :hint="hint">
            <v-layout row wrap class="fluro-academic-field">
                <v-flex xs12 sm6>
                    <v-select :outline="showOutline" :success="success" :required="required" label="School / Institution" no-data-text="No options available" v-model="modelProxy" item-text="title" item-value="_id" :items="calendars" :return-object="true" :error-messages="errorMessages" @input="calendarChanged" />
                </v-flex>
...
export default {
    ...
    computed: {
        modelProxy: {
            get() {
                return this.model.academicCalendar;
            },
            set(value) {

                if (!value || !value._id || !value._id.length) {
                 console.log('Set Proxy', null);
                    this.$set(this.model, 'academicCalendar', null);
                } else {
                 console.log('Set Proxy', value);
                    this.$set(this.model, 'academicCalendar', value);
                }
        ...
        grades() {
            var self = this;
            return self.model.academicCalendar ?
                self.model.academicCalendar.grades : [];
        }
    },
    created() {
        if (this.formFields) {
