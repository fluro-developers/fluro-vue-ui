<template>
	<div
		@click="clicked($event)"
		:data-field-key="key"
		class="fluro-content-form-field"
		v-if="isVisible"
		v-bind="attributes"
		:class="fieldClass"
	>
		...
		<template v-if="ready && model">
			<template v-if="officeUseOnly"> </template>
				...
				<template v-if="asObject">
					<template v-if="!multipleInput">
						<fluro-content-form
							...
							:options="options"
							v-model="fieldModel"
							@input="elementValueChanged($event, true, 'emit')"
							:fields="fields"
						/>
					</template>
					<template v-else>
			...
			<template v-else-if="renderer == 'number'">
				<!-- type="number" -->
				<v-text-field
					:persistent-hint="persistentDescription"
					:suffix="suffix"
					:prefix="prefix"
					:outline="showOutline"
					:success="success"
					...
					<v-layout>
						<v-flex>
							<v-select
								...
								:success="success"
								:required="required"
								v-model="dateModelDay"
								item-text="title"
								:items="dateDayOptions"
								@blur="touch()"
								@focus="focussed()"
							/>
						</v-flex>
						...
						<v-flex>
							<v-select
								...
								:success="success"
								:required="required"
								v-model="dateModelMonth"
								item-text="title"
								:items="dateMonthOptions"
								@blur="touch()"
								@focus="focussed()"
							/>
						</v-flex>
						...
						<v-flex>
							...
							<v-select
								...
								:success="success"
								:required="required"
								v-model="dateModelYear"
								item-text="title"
								:items="dateYearOptions"
								@blur="touch()"
								@focus="focussed()"
							/>
						</v-flex>
					...
					<div class="button-select-buttons">
						<template v-if="webMode">
							<fluro-button
								...
								@click.native="toggleValue(button.value)"
								:class="{
									active: isSelectedValue(button.value),
									inactive: !isSelectedValue(button.value),
								}"
								:key="button.value"
						...
						<template v-else>
							<v-btn
								:block="$vuetify.breakpoint.xsOnly"
								:color="isSelectedValue(button.value) ? `primary` : null"
								:class="{ active: isSelectedValue(button.value) }"
								class="ml-0"
								@click.native="toggleValue(button.value)"
								v-for="button in selectOptions"
							>
				...
				>
					<template v-slot:activator="{ on }">
						...
						<v-text-field
							@blur="touch()"
							@focus="focussed()"
							@keyup.enter="checkTextDateImmediate"
							@input="checkTextDate"
							:outline="showOutline"
							:success="success"
							v-model="textDate"
			...
			<template v-else-if="renderer == 'search-select'">
				<v-autocomplete
					...
					:error-messages="errorMessages"
					:cache-items="!defaultReferences || !defaultReferences.length"
					:chips="multipleInput"
					:clearable="!required"
					:return-object="true"
					item-text="title"
			...
			<template v-else-if="renderer == 'textarea'">
				<v-input
					...
					:outline="showOutline"
					:success="success"
					:error-messages="multipleInput ? errorMessages : null"
				>
					<template v-if="multipleInput">
						<template v-if="fieldModel && fieldModel.length">
							<template v-for="(entry, index) in fieldModel">
					...
					<template v-slot:activator="{ on }">
						<v-input :label="displayLabel" :persistent-hint="true" class="no-flex">
							<div class="color-swatch" v-on="on">
								<div class="swatch" :style="{ backgroundColor: colorModel.hex8 }">
									<span>{{ colorModel.hex8 ? colorModel.hex8 : 'transparent' }}</span>
								</div>
							</div>
						</v-input>
					</template>
			...
			<template v-else>
				<template v-if="multipleInput">
					<v-input :outline="showOutline" :label="displayLabel" :success="success" class="no-flex">
						...
						<template v-if="canAddValue">
							<v-text-field
								:type="directive == 'password' ? 'password' : 'text'"
								@focus="focussed()"
								:mask="params.mask"
								:autofocus="shouldAutofocus"
								class="faint"
								...
								@keyup.enter.native.stop.prevent="addProposedValue()"
								@blur="addProposedValue()"
								:error-messages="errorMessages"
								:persistent-hint="persistentDescription"
								:hint="hint"
								:placeholder="placeholder"
...
function createDynamicComponent(globalComponent, extraComponents) {
	...
	try {
		...
		console.log(
			'Error generating dynamic component',
			globalComponent.title,
			globalComponent.key,
			e,
			globalComponent.js
		);
...
export default {
	...
	watch: {
		...
		expressionDefaultValue(v) {
			...
			console.log('expression default value', this.field.key, v);
		...
		expressionValue(v) {
			...
			console.log('expression value', this.field.key, v);
			this.$set(this.model, this.field.key, v);
			this.$emit('input', this.model);
		},
	...
	computed: {
		...
		dateModelDay: {
			...
			set(v) {
				...
				this.dateStringModel = `${this.actualDateModelYear}-${zeroPadded(
					this.actualDateModelMonth
				)}-${zeroPadded(this.actualDateModelDay)}`;
			},
		},
		dateModelMonth: {
			get() {
				...
				case 'date':
					if (value) {
						if (String(value).toLowerCase() == 'now') {
							...
							var isValidDate = d instanceof Date && !isNaN(d);
	...
	beforeCreate() {
		...
		if (
			self.$fluro.app &&
			self.$fluro.app.site &&
			self.$fluro.app.site.components &&
	...
	if (typeof minimumValue != 'undefined') {
		...
		if (input < minimumValue) {
			return `Must be at least ${
				currency && self ? self.$fluro.utils.formatCurrency(minimumValue, currency) : minimumValue
			}`;
		}
	}
	...
	if (typeof maximumValue != 'undefined' && parseInt(maximumValue) != 0) {
		...
		if (input > maximumValue) {
			return `Must be less than ${
				currency && self ? self.$fluro.utils.formatCurrency(maximumValue, currency) : maximumValue
			}`;
		}
	}
}
