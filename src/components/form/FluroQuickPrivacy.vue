<template>
  ...
  <v-btn @click="toggle()" v-tippy :content="tooltip" :small="small" :block="block">
    <span :class="{'text-muted':model.privacy == 'public'}">
      <fluro-icon :icon="icon" :library="model.privacy  =='public' ? 'far' : 'fas'" />
    </span>
  </v-btn>
  <!-- </template> -->
  <!-- <v-card tile>
  ...
  computed: {
    definitionTitle() {
      return this.$fluro.types.readable(
        this.model.definition || this.model._type || "node"
      );
    },
    tooltip() {
      return this.model.privacy == "public"
        ? `Public - Anyone can view this ${this.definitionTitle}`
        : `Secure - This ${this.definitionTitle} is secured by permissions`;
    },
    icon() {
