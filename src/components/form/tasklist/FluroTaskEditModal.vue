		...
		<flex-column-header class="border-bottom">
			<v-container pa-2>
				<v-layout align-center row>
					<v-flex>
						<h4><fluro-icon icon="square-pencil" /> {{ model.name }}</h4>
					</v-flex>
					<v-flex shrink>
						<v-btn class="ma-0" small color="primary" @click="close(model)"> Done </v-btn>
					</v-flex>
				</v-layout>
			</v-container>
		</flex-column-header>
							...
							fields: [
								{
									title: 'Pending Label',
									description:
										'Set what label should be used when referring to this task being in progress or',
									key: 'pendingLabel',
									params: {
										persistentDescription: true,
									},
								...
								},
								{
									title: 'Instructions',
									description:
										'Add extra detail about what this task means when it is marked as pending',
