<template>
	<div class="fluro-task-list">
		...
		>
			<fluro-task-item
				:state="state"
				:mode="editMode ? 'edit' : 'do'"
				@remove="removeTask"
				:definition="definition"
				:card="card"
				v-model="model.tasks[index]"
				@input="update"
				:key="task.guid"
				...
				<template v-slot:edit="{ props, blur, focus }">
					<v-layout row>
						<v-flex>
							<input
								block
								v-model="proposedTaskName"
								@blur="blur"
								placeholder="Type a title for this task and then press enter"
								@keyup.enter="pushProposedTask()"
							/>
						</v-flex>
