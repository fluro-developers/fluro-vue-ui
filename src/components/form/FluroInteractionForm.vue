							...
							<div class="payment" v-if="showPaymentForm">
								...
								<div class="border-top">
									<tabset :justified="true" v-model="selectedPaymentMethod">
											...
											<template v-else>
												<v-container style="background: #fff" class="border-top">
													...
													>
														...
														<strong>{{
															actualPaymentIntegration.displayTitle ||
															actualPaymentIntegration.title
														}}</strong>
													</div>
													<fluro-content-form
														@errorMessages="validate"
										...
										<tab
											:heading="paymentMethod.title"
											v-for="paymentMethod in alternativePaymentMethods"
		...
		options: {
			default: function () {
				...
				return {
					...
					backupUploadRealm: _.chain(self.definition).get('realms').first().value(),
				};
			},
			type: Object,
		},
	...
	created() {
		...
		if (self.options) {
			if (!self.options.backupUploadRealm) {
				self.options.backupUploadRealm = _.chain(self.definition).get('realms').first().value();
