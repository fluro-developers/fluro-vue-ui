	...
	<div class="fluro-editor">
			...
			>
						...
						<template v-if="proEnabled">
							<template v-if="constrain">
								...
								<input
									...
									placeholder="100"
									ref="widthInput"
									@change="scaleImage(commands.image)"
									@blur="commands.image(selectedImage)"
								/>
							</template>
							<template v-else>
				...
				<template v-else-if="selectedVideo">
					<form
						...
						v-if="isActive.video()"
						@submit.prevent.stop="scaleVideo(commands.video)"
					>
						<!-- This function stops the page being submitted -->
						<label for="widthInput">&nbsp;Scale:&nbsp;</label>
						<input
							...
							placeholder="100"
							ref="widthInput"
							@change="scaleVideo(commands.video)"
							@blur="commands.video(selectedVideo)"
						/>
					</form>
				</template>
				<template v-else>
					<v-menu attach v-if="isEnabled('formats')" transition="slide-y-transition" offset-y>
						...
						<div style="overflow: auto; width: 300px; max-height: 300px">
							<v-list>
								...
								>
									<v-list-tile-content
										><span style="margin: 0 !important" class="h1"
											>Heading 1</span
										></v-list-tile-content
									>
								</v-list-tile>
								...
								>
									<v-list-tile-content
										><span style="margin: 0 !important" class="h2"
											>Heading 2</span
										></v-list-tile-content
									>
								</v-list-tile>
								...
								>
									<v-list-tile-content
										><span style="margin: 0 !important" class="h3"
											>Heading 3</span
										></v-list-tile-content
									>
								</v-list-tile>
								<v-list-tile
									...
									@click.stop.prevent="commands.heading({ level: 4 })"
								>
									<v-list-tile-content
										><span style="margin: 0 !important" class="h4"
											>Heading 4</span
										></v-list-tile-content
									>
								</v-list-tile>
								<v-list-tile
									...
									@click.stop.prevent="commands.heading({ level: 5 })"
								>
									<v-list-tile-content
										><span style="margin: 0 !important" class="h5"
											>Heading 5</span
										></v-list-tile-content
									>
								</v-list-tile>
								<v-list-tile
									:class="{ active: isActive.typography({ level: option.level }) }"
									v-for="option in typographyOptions"
									:key="option.title"
									@click.stop.prevent="commands.typography({ level: option.level })"
								>
									<v-list-tile-content
										><span style="margin: 0 !important" :class="option.level">{{
											option.title
										}}</span></v-list-tile-content
									>
								</v-list-tile>
								<template v-if="false">
									<v-list-tile
										@click.stop.prevent="commands.fluroNode(option)"
										v-for="option in getFluroNodes()"
										:key="option.title"
									>
										<v-list-tile-content
											><span style="margin: 0 !important" :class="option.className">{{
												option.title
											}}</span></v-list-tile-content
										>
									</v-list-tile>
								...
								<template v-if="false">
									<v-list-tile
										@click.stop.prevent="commands.fluroMark(option)"
										v-for="option in getFluroMarks()"
										:key="option.title"
									>
										<v-list-tile-content
											><span style="margin: 0 !important" :class="option.class">{{
												option.title
											}}</span></v-list-tile-content
										>
									</v-list-tile>
					...
					<template v-if="isEnabled('alignment')">
						<v-btn
							...
							small
							flat
							:class="{ active: isActive.alignment({ textAlign: 'left' }) }"
							@click.stop.prevent="commands.alignment({ textAlign: 'left' })"
						>
						...
						<v-btn
							...
							small
							flat
							:class="{ active: isActive.alignment({ textAlign: 'center' }) }"
							@click.stop.prevent="commands.alignment({ textAlign: 'center' })"
						>
						...
						<v-btn
							...
							small
							flat
							:class="{ active: isActive.alignment({ textAlign: 'right' }) }"
							@click.stop.prevent="commands.alignment({ textAlign: 'right' })"
						>
					...
					<template v-if="isActive.link()">
						<v-btn
							...
							small
							flat
							@click.stop.prevent="showLinkMenu(commands.link, getMarkAttrs('link'))"
						>
							<!-- Link Options -->
					...
					<v-btn
						...
						small
						flat
						@click.stop.prevent="addLinkMenu(commands.link, getMarkAttrs('link'))"
					>
						<!-- Add Link -->
						<!-- <span>{{ isActive.link() ? 'Update Link' : 'Link'}}</span> -->
						<fluro-icon icon="link" />
		...
		<editor-menu-bar :editor="editor" v-if="barEnabled">
			<div class="fluro-editor-toolbar" slot-scope="{ commands, isActive }">
				<v-btn
					v-if="isEnabled('source')"
					icon
					small
					flat
					class="hidden-xs-only"
					:class="{ 'is-active': showSource }"
					@click.stop.prevent="showSource = !showSource"
				>
				...
				<v-menu v-if="isEnabled('formats')" :fixed="true" transition="slide-y-transition" offset-y>
					...
					<v-list>
						...
						>
							<v-list-tile-content
								><span style="margin: 0 !important" class="h1">Heading 1</span></v-list-tile-content
							>
						</v-list-tile>
						<v-list-tile
							:class="{ active: isActive.heading({ level: 2 }) }"
						...
						>
							<v-list-tile-content
								><span style="margin: 0 !important" class="h2">Heading 2</span></v-list-tile-content
							>
						</v-list-tile>
						<v-list-tile
							:class="{ active: isActive.heading({ level: 3 }) }"
						...
						>
							<v-list-tile-content
								><span style="margin: 0 !important" class="h3">Heading 3</span></v-list-tile-content
							>
						</v-list-tile>
						<v-list-tile
							:class="{ active: isActive.heading({ level: 4 }) }"
						...
						>
							<v-list-tile-content
								><span style="margin: 0 !important" class="h4">Heading 4</span></v-list-tile-content
							>
						</v-list-tile>
						<v-list-tile
							:class="{ active: isActive.heading({ level: 5 }) }"
						...
						>
							<v-list-tile-content
								><span style="margin: 0 !important" class="h5">Heading 5</span></v-list-tile-content
							>
						</v-list-tile>
						<v-list-tile
							:class="{ active: isActive.typography({ level: option.level }) }"
				...
				<v-menu v-if="tokens.length" :fixed="true" transition="slide-y-transition" offset-y>
					<template v-slot:activator="{ on }">
						<v-btn small :disabled="showSource" v-on="on"> Tokens </v-btn>
					</template>
					<v-list dense>
						<v-list-tile @click="commands.token(token.key)" :key="token.key" v-for="token in tokens">
							<v-list-tile-content
								><span style="margin: 0 !important">{{ token.title }}</span></v-list-tile-content
							>
						</v-list-tile>
					</v-list>
				</v-menu>
				<v-menu
					v-if="
						...
						isEnabled('code_block') ||
						isEnabled('horizontal_rule')
					"
					:fixed="true"
				...
				>
					...
					<v-list dense>
						...
						<v-list-tile v-if="isEnabled('image')" @click.stop.prevent="showImagePrompt(commands.image)">
							<v-list-tile-content
								><span style="margin: 0 !important">
									<fluro-icon type="image" />&nbsp;Add Image</span
								></v-list-tile-content
							>
						</v-list-tile>
						<v-list-tile v-if="isEnabled('video')" @click.stop.prevent="showVideoPrompt(commands.video)">
							<v-list-tile-content
								><span style="margin: 0 !important">
									<fluro-icon type="video" />&nbsp;Add Video</span
								></v-list-tile-content
							>
						</v-list-tile>
						<v-list-tile
						...
						>
							<v-list-tile-content
								><span style="margin: 0 !important">
									<fluro-icon icon="quote-right" />&nbsp;Blockquote</span
								></v-list-tile-content
							>
						</v-list-tile>
						<v-list-tile
						...
						>
							<v-list-tile-content
								><span style="margin: 0 !important">
									<fluro-icon icon="code" />&nbsp;Code</span
								></v-list-tile-content
							>
						</v-list-tile>
						<v-list-tile
							:class="{ active: isActive.horizontal_rule() }"
							@click.stop.prevent="commands.horizontal_rule"
						>
							<v-list-tile-content
								><span style="margin: 0 !important">
									<fluro-icon icon="horizontal-rule" />&nbsp;Horizontal Rule</span
								></v-list-tile-content
							>
						</v-list-tile>
					</v-list>
					...
					<v-list dense>
						...
						<span v-if="isActive.table()">
							...
							<v-list-tile @click.stop.prevent="commands.addRowBefore">
								<v-list-tile-content> Insert Row Before </v-list-tile-content>
							</v-list-tile>
							<v-list-tile @click.stop.prevent="commands.addRowAfter">
								<v-list-tile-content> Insert Row After </v-list-tile-content>
							</v-list-tile>
							<v-list-tile @click.stop.prevent="commands.deleteRow">
								<v-list-tile-content> Delete Row </v-list-tile-content>
							</v-list-tile>
							<!-- <v-subheader>Column</v-subheader> -->
							<v-list-tile @click.stop.prevent="commands.addColumnBefore">
								<v-list-tile-content> Insert Column Before </v-list-tile-content>
							</v-list-tile>
							<v-list-tile @click.stop.prevent="commands.addColumnAfter">
								<v-list-tile-content> Insert Column After </v-list-tile-content>
							</v-list-tile>
							<v-list-tile @click.stop.prevent="commands.deleteColumn">
								<v-list-tile-content> Delete Column </v-list-tile-content>
							</v-list-tile>
							<v-divider></v-divider>
							<v-divider></v-divider>
							<!-- <v-subheader>Actions</v-subheader> -->
							<v-list-tile @click.stop.prevent="commands.toggleCellMerge">
								<v-list-tile-content> Combine Cells </v-list-tile-content>
							</v-list-tile>
							<v-divider></v-divider>
							<v-list-tile @click.stop.prevent="commands.deleteTable">
								<v-list-tile-content> Delete table </v-list-tile-content>
							</v-list-tile>
						</span>
					</v-list>
				</v-menu>
		...
		<template v-if="!showSource">
			...
			<div class="suggestion-list" v-show="showSuggestions" ref="suggestions">
				<template v-if="filteredUsers.length">
					...
					<div
						v-for="(persona, index) in filteredUsers"
						:key="persona._id"
						class="suggestion-list__item"
						:class="{ 'is-selected': navigatedUserIndex === index }"
						@click.stop.prevent="selectUser(persona)"
					>
...
export default {
	...
	created() {
		...
		var MentionExtension = new Mention({
			...
			// is called when a suggestion has changed
			onChange: _.debounce(async ({ items, query, range, virtualNode }) => {
				var scope = this;

				return new Promise(function (resolve, reject) {
					self.$fluro.content
						.mention(query, self.options.mentions)
						.then(function (items) {
		...
		if (process.browser) {
			if (window.hljs) {
				var json = function () {
					return window.hljs.getLanguage('json');
				};
				var javascript = function () {
					return window.hljs.getLanguage('javascript');
				};
				var scss = function () {
					return window.hljs.getLanguage('scss');
				};
