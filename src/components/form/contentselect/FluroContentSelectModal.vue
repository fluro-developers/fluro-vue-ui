				...
				<v-layout fill-height>
					<flex-column style="min-height: 50vh">
						...
						<fluro-dynamic-table
							:startDate="startDate"
							:endDate="endDate"
							:includeArchivedByDefault="includeArchivedByDefault"
							:additionalKeys="joins"
							:queryString="false"
					...
					<flex-column class="filter-sidebar" v-show="showFilters">
						<flex-column-body>
							...
							<v-container pa-2 v-if="dateFilterVisible">
								<help
									title="Date filters"
									body="Filter records by providing a relevant date range"
										...
										<v-layout>
											...
											<v-flex>
												<v-card-text class="text-sm-center"
													><label>From Date</label></v-card-text
												>
												<v-date-picker
													v-model="startDateString"
													no-title
...
export default {
	...
	computed: {
		...
		multiType() {
			var hasSubDefinitions = this.parentType == this.type && this.retrieveAllDefinitions;
					...
					array = array.concat([
						{
							...
							key: 'title',
							renderer: TitleCell,
							additionalFields: ['event', 'startDate', 'endDate'],
						},
					]);
					break;
				case 'family':
				...
				case 'contact':
					array = array.concat([
						{ title: '', key: '_id', renderer: TypeImageCell, shrink: true },
						{
							...
							key: 'title',
							renderer: TitleCell,
							additionalFields: ['firstName', 'preferredName', 'lastName'],
						},
