        ...
        <div class="actions">
            ...
            <template v-else>
                ...
                <slot name="submit" :hasErrors="hasErrors">
                    <template v-if="webMode">
                        <!-- type="submit" -->
                        <fluro-button tag="button" @click.native.prevent="submit" type="submit" :disabled="hasErrors">{{submitButtonText}}</fluro-button>
                    </template>
                    <template v-else>
                        <!--  -->
                        <v-btn class="mx-0" type="submit" :disabled="hasErrors" color="primary">{{submitButtonText}}</v-btn>
                    </template>
                </slot>
            </template>
        </div>
