			...
			<v-container>
				<constrain sm>
					<wrapper xs>
						...
						<div class="muted">
							<strong
								><span class="text-capitalize">{{ item.mailoutType }}</span> mailout sent from
								{{ item.fromEmail }}</strong
							>
						</div>
						<div>
							<strong>{{ emails.length }} Recipients in total</strong>
						</div>
					</wrapper>
					<wrapper xs>
						<v-layout row wrap>
					...
					<template v-else>
						...
						<fluro-panel v-if="urls.length">
							<fluro-panel-title> Top Links </fluro-panel-title>
							<div>
								<a class="row-item" :href="url.url" target="_blank" v-for="url in urls">
									<v-layout align-center>
										<v-flex class="small">{{ url.url }}</v-flex>
										<div>
											<strong>{{ url.count }}</strong> clicks
											<span class="muted">({{ url.percent }}%)</span>
										</div>
									</v-layout>
								</a>
							</div>
						...
						<fluro-panel>
							<fluro-panel-body class="border-bottom">
								<v-layout>
									...
									<div>
										<v-btn @click="push()" color="primary" class="ma-0 ml-2"
											>Push
							...
							<div style="background: #fff">
								<tabset>
									...
									>
										<fluro-table
											trackingKey="correspondenceID"
											defaultSort="firstName"
											:pageSize="25"
											style="max-height: 50vh"
											:items="segment.emails"
											:columns="columns"
										/>
									</tab>
								</tabset>
...
export default {
	...
	methods: {
		...
		loadCorrespondenceResults() {
			...
			self.polling = true;
			self.$fluro.api
				.get(`/mailout/${this.item._id}/correspondence`, {
					params: {
						timestamp: new Date(),
					},
				})
