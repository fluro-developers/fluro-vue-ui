				...
				<constrain sm>
					<wrapper xs>
						<template v-if="item.state == 'scheduled'">
							...
							<strong v-if="item.publishDate">{{
								item.publishDate | formatDate('h:mma - dddd D MMM YYYY')
							}}</strong>
						</template>
						<template v-else>
						...
						<div v-else>
							<p class="lead">
								Sending to {{ validContacts.length }} of {{ preflightContacts.length }} selected
								contacts
							</p>
						</div>
					</wrapper>
					<wrapper xs v-if="preflightContacts.length">
								...
								<v-layout align-center v-for="contact in invalidSummary">
									<v-flex>
										<strong class="text-capitalize"
											>{{ contact.firstName }} {{ contact.lastName }}</strong
										>
									</v-flex>
						...
						<v-alert v-if="unsubscribedContacts.length" :value="true" type="error" outline>
							<div @click="expanded.unsubscribed = !expanded.unsubscribed">
								<h5>
									<em
										>{{ unsubscribedContacts.length }} selected contacts have unsubscribed from mail
										of this type
									</em>
								</h5>
							</div>
	...
	methods: {
		publish() {
			...
			function publishConfirmed() {
				...
				self.$fluro.api
					.get(`/mailout/${self.item._id}/publish`, {
						params: {
							date: publishDate,
						},
					})
		...
		abort() {
			...
			self.$fluro.api
				.get(`/mailout/${self.item._id}/abort`)
				.then(function (res) {
		...
		preflight() {
			...
			self.$fluro.api
				.get(`/mailout/${this.item._id}/preflight`, { cache: false })
				.then(function (res) {
