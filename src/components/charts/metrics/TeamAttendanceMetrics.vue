		...
		<template v-if="model">
			...
			<fluro-panel v-if="eventData.model.series.attendance.length">
				<tabset>
					...
					<tab heading="Ticks">
						<div class="dots-view">
							<fluro-table :disablePageScroll="true" :pageSize="30" defaultSort="lastName" :items="grouped" :columns="dotColumns"></fluro-table>
						</div>
					</tab>
				...
				<template v-slot:activator="{ on }">
					<div v-on="on">
						<template v-if="selectedEvent">
							<h4>
								{{selectedEvent.title}} <span class="muted">{{(selectedEvent.definition || 'event') | definitionTitle}}</span> <span class="muted">
									<fluro-icon icon="angle-down" /></span>
							</h4>
							<div class="muted">{{selectedEvent | readableEventDate}} ({{selectedEvent.startDate | timeago}})</div>
						</template>
						<template v-else>
							<h4>
								Select Date <span class="muted">
									<fluro-icon icon="angle-down" /></span>
							</h4>
			...
			<template v-if="selectedEvent">
						...
						<template v-for="post in selectedEvent.posts">
							<fluro-panel-title class="border-top">
								<v-layout align-center>
									...
									<v-flex shrink>
										<v-btn icon small class="ma-0" v-if="$fluro.access.canEditItem(post)" @click="editPost(post)">
							...
							<fluro-content-view :hideTitle="true" :id="post._id" :embedded="true" :definition="post.definition" :type="post._type" />
						</template>
						<div v-if="hasCheckins" class="border-top">
							...
							<tabset :justified="true">
								...
								<tab :heading="`${checkedIn.length} Checked In`">
									<!-- <div style="background: #fff" class="border-top"> -->
									<fluro-table :disablePageScroll="true" :pageSize="10" :items="checkedIn" :columns="columns"></fluro-table>
								</tab>
								<tab :heading="`${absent.length} Absent`">
									<!-- <div style="background: #fff" class="border-top"> -->
									<fluro-table :disablePageScroll="true" :pageSize="10" :items="absent" :columns="columns"></fluro-table>
								</tab>
							</tabset>
							<!-- </div> -->
						</div>
				...
				<fluro-panel v-else>
					<fluro-panel-body>
						<div class="muted text-xs-center">No data for {{selectedEvent.startDate | formatDate('h:mma - ddd D MMM YYYY')}}</div>
					</fluro-panel-body>
				</fluro-panel>
			</template>
...
export default {
  ...
  computed: {
    ...
    grouped() {
      ...
      return _.chain(self.model)
        ...
        .map(function (entry) {
          ...
          entry.percentWeeks = Math.ceil(
            (entry.total / _.keys(self.weeks).length) * 100
          );
          entry.percentMonths = Math.ceil(
            (entry.total / _.keys(self.months).length) * 100
          );
          return entry;
        })
    ...
    allMembers() {
      return [].concat(this.leaders, this.provisionalMembers);
    },
    membersLookup() {
      return _.reduce(
        this.allMembers,
    ...
    hasPosts() {
      return (
        this.selectedEvent &&
        this.selectedEvent.posts &&
        this.selectedEvent.posts.length
      );
    },
    hasCheckins() {
      return (
        this.selectedEvent &&
        this.selectedEvent.checkins &&
        this.selectedEvent.checkins.length
      );
    },
    selectedEvent: {
      get() {
    ...
    eventData() {
      ...
      var max = Math.max(
        _.max(model.series.attendance),
        _.max(model.series.expected)
      );
    ...
    attendance() {
      ...
      _.each(allContacts, function (contact) {
        ...
        var assignedContacts = _.map(self.group.assignments, function (
          assignment
        ) {
        ...
        if (
          _.find(assignedContacts, function (assignment) {
      ...
      allContacts = _.sortBy(allContacts, [
        "expected",
        "lastName",
        "firstName",
      ]);
