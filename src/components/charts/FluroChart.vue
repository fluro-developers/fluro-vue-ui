				...
				computed: {
								...
								groupChartData: function() {
												...
												function groupData(groupFormat, datasource) {
																...
																return _.reduce(dataSource.axis, function(set, entryDate, key) {
																				...
																				_.each(self.series, function(ser) {
																								var currentTotal = _.get(existing, ser.key) || 0
																								existing[ser.key] = currentTotal + _.get(datasource, `series.${ser.key}`)[key];
												...
												_.each(groupedData, function(entry, key) {
																...
																// returnData.axis.push(moment(key).format())
																_.each(self.series, function(ser) {
				...
				methods: {
								...
								getGroupDate(date, format) {
									...
									var self = this;
												switch (format) {
																case '4week':
																				var weekNumber = moment(date).format('WW');
																				var yearNumber = moment(date).format('YYYY');

																				var weekPeriod = Math.ceil(weekNumber / 4);
																				// var periodName = `PERIOD ${weekPeriod} ${yearNumber}`;
																				var d = moment().year(parseInt(yearNumber)).week(parseInt(weekPeriod * 4)).startOf('isoweek').weekday(self.weekstart).toDate();
																				// (periodName, 'W YYYY').startOf('week').toDate();
																				console.log('D', 'week:', weekNumber, 'period:', weekPeriod, 'name:', d);
																				return d;
																				break;
																case '6week':
																				var weekNumber = moment(date).format('WW');
																				var yearNumber = moment(date).format('YYYY');
																				var weekPeriod = Math.ceil(weekNumber / 6);
																				// var periodName = `PERIOD ${weekPeriod} ${yearNumber}`;
																				var d = moment().year(parseInt(yearNumber)).week(parseInt(weekPeriod * 6)).startOf('isoweek').weekday(self.weekstart).toDate();
																				// (periodName, 'W YYYY').startOf('week').toDate();
																				console.log('D', 'week:', weekNumber, 'period:', weekPeriod, 'name:', d);
																				return d;
																				break;
																case 'week':
																				return moment(date).startOf('isoweek').weekday(self.weekstart).toDate()
																				break;
																case 'quarter':
																case 'month':
																case 'year':
