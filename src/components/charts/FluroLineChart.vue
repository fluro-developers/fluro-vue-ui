		...
		<div v-if="chartOptions">
			...
			<div v-if="isBrushed" id="brushedChartKey">
				<apexchart
					:type="chartType"
					:width="width"
					:height="brushedChartOptions.chart.height"
					:options="brushedChartOptions"
					:series="brushedChartData"
				></apexchart>
			</div>
		</div>
			...
			return new Promise(function (resolve, reject) {
				...
				if (!_.get(chartOpt, 'annotations')) {
					if (self.model.axis.key.toLowerCase().includes('date')) {
						...
						var defaultAnnotations = _.chain(years)
							.map(function (year) {
								return [
									{
