		...
		<flex-column-header class="border-bottom">
			<div class="calendar-header">
				<v-layout align-center>
					...
					<v-flex shrink>
						<v-btn small @click="today()"> Today </v-btn>
			...
			<v-calendar ref="calendar" v-model="nowDateString" :type="calendar.value" color="primary">
					...
					<template v-for="event in eventMap[date]">
						<div class="event-month-item" @click="selectEvent(event)">
							...
							<v-layout>
								...
								<v-flex>
									<span class="time">{{ event.readableTime }} </span>
				...
				<template v-slot:dayHeader="{ date }">
					<template v-for="event in eventMap[date]" v-if="event.multiDay">
						...
						<div
							...
							class="event-calendar-pill single-line"
						>
				...
				<template v-slot:dayBody="{ date, timeToY, minutesToPixels }">
					<template v-if="timeMap[date]">
						<div
							class="timeslot"
							v-for="block in timeMap[date].slots"
							:style="{ top: timeToY(block.time) + 'px', height: minutesToPixels(block.duration) + 'px' }"
							:class="[`indent-${block.indent}`]"
						>
							<!-- full-width -->
							...
							<v-layout>
								...
								<v-flex :key="event.guid" v-for="event in block.events">
									...
									>
										{{ event.title }}
										<div class="time" v-if="!event.multiDay">
											{{ event.startDate | formatDate('h:mma') }}
										</div>
				...
				.reduce(
					function (set, event) {
						...
						if (!existingMonthEntry) {
							existingMonthEntry = set.months[month] = {
								title: month,
								dates: {},
							};
						}
						...
						if (!existingDateEntry) {
							existingDateEntry = set.events[eventDate] = set.months[month].dates[eventDate] = [];
						}
