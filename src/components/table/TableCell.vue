<template>
	<td
		...
		:class="{
			...
			'no-padding': column.padding === false,
		}"
	>
			...
			<div v-else-if="formattedArray" class="formatted-array">
				<template v-for="entry in formattedArray">
					<a
						class="inline-tag"
						v-if="entry._id"
						@click.stop.prevent="clicked(entry)"
						:style="{ color: entry.color, backgroundColor: entry.bgColor }"
					>
						<template v-if="entry._type == 'event'">
							<fluro-icon type="event" /> {{ entry.title }}
							<span class="text-muted">// {{ entry | readableEventDate }}</span>
						</template>
						<template v-else-if="entry._type == 'unavailability'">
							<fluro-icon type="event" /> {{ entry.title }}
							<span class="">- {{ entry | readableEventDate }}</span>
						</template>
						<template v-else-if="entry._type == 'post'">
							<fluro-icon type="post" /> {{ entry.title }}
							<span class="text-muted"
								>// {{ entry.managedAuthor ? entry.managedAuthor.title : '' }}
								{{ entry.created | timeago }}</span
							>
						</template>
						<template v-else-if="entry._type == 'ticket'">
							<fluro-icon type="ticket" /> {{ entry.title }} - {{ entry.event.title
							}}<span class="text-muted">// {{ entry.event | readableEventDate }}</span>
						</template>
						...
						<template v-else>
							<fluro-icon v-if="entry._type" :type="entry._type" /> {{ entry.title }}
							<template v-if="entry.appendage"> - {{ entry.appendage }}</template>
						</template>
					</a>
					<a
						class="inline-tag"
						v-else-if="entry._type == 'log'"
						@click.stop.prevent="$fluro.global.json(entry)"
					>
						<fluro-icon icon="clock" /> {{ entry.title }}
						<span class="text-muted">// {{ entry.created | timeago }}</span>
					</a>
			...
			<div v-else-if="complexObject">
				<template v-if="preValue._type == 'event'">
					<a class="inline-tag" @click.stop.prevent="clicked(preValue)">
						<fluro-icon type="event" /> {{ preValue.title }}
						<span class="text-muted">// {{ preValue | readableEventDate }}</span>
					</a>
				</template>
				...
				<template v-else>
					<!--  TESTING WEIRD -->
					<a
						@click.stop.prevent="clicked(preValue)"
						class="inline-tag"
						:style="{ color: preValue.color, backgroundColor: preValue.bgColor }"
					>
						<fluro-icon v-if="preValue._type" :type="preValue._type" /> {{ complexTitle }}
						<template v-if="complexAppendage"> - {{ complexAppendage }}</template>
					</a>
				</template>
			</div>
				...
				var array = _.chain(val)
					...
					.filter(function (v) {
						...
						if (definitionDiscriminator) {
							return (
								v.definition == definitionDiscriminator ||
								v._discriminator == definitionDiscriminator ||
								v._discriminatorType == definitionDiscriminator
							);
						}
