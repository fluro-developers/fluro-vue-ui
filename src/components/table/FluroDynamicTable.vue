<template>
 ...
 -->
		...
		<template v-else>
			<div
				ref="scrollableArea"
				class="fluro-table-scroll"
				:class="{ clickable: clickable, loading: showLoading }"
			>
				<span ref="top"></span>
					...
					<thead>
						<tr>
							<th class="first shrink checkbox-cell" v-if="selectionEnabled">
								<fluro-help
									title="Select all"
									body="This checkbox allows you to select all items in this table"
								/>
									...
									<v-card class="select-options" tile>
										<v-list dense>
											<v-list-tile v-if="!allSelected" @click="selectPage()">
												<v-list-tile-content>
													<v-list-tile-title> Select this page </v-list-tile-title>
												</v-list-tile-content>
											</v-list-tile>
											...
											<v-list-tile
												@click="deselectAll()"
												v-if="selectionController.selection.length"
											>
												<v-list-tile-content>
													<v-list-tile-title>
							...
							<th
								@contextmenu.prevent="showOptionsForColumn(column)"
								@click="toggleSort(column)"
								:class="{
									shrink: column.shrink,
									sortable: isSortable(column),
									sorting: sort.sortKey == column.key,
									tiny: column.tiny,
									'text-xs-center': column.align == 'center',
									'text-xs-right': column.align == 'right',
								}"
							...
							<th class="last shrink" v-if="enableActions">
								<v-menu :close-on-content-click="false" @click.native.stop offset-y>
									<template v-slot:activator="{ on }">
										<div v-on="on">
											<fluro-help
												...
												body="You can export, print and customise the rendered columns from here"
											/>
									...
									<v-card tile>
										...
										<v-list style="max-height: 50vh" class="scroll-y" dense>
											<v-list-tile
												:class="{ columnselected: columnIsSelected(field) }"
												@click="toggleColumn(field)"
												v-for="field in availableKeys"
											>
												<v-list-tile-action>
							...
							<template v-for="group in grouped">
								<tr class="breaker">
									...
									<template v-else>
										<th
											...
											class="first"
											is="table-row-checkbox"
											@click.native="toggleSet(group.items)"
											:checked="allInGroupSelected(group.items)"
										/>
										<th />
									</template>
									<!-- <td></td> -->
									<!-- <th is="table-row-checkbox" :checked="selectionController.allSelected(group)"/> -->
									<td :colspan="1 + renderColumns.length">
										{{ group.title || 'No value' }}
										<span class="sm muted">({{ group.items.length }})</span>
									</td>
								</tr>
								<tr :class="classes(item)" :key="item.guid" v-for="(item, key) in group.items">
									<template v-if="item._populating">
										...
										<td @click.native="clicked(item, column, key)" :colspan="renderColumns.length">
											-
										</td>
										<th class="last shrink" v-if="actionsEnabled"></th>
									</template>
									<template v-else>
										<th
											class="first"
											...
											:checked="selectionController.isSelected(item)"
											@click.native.stop.prevent="checkboxClick(item, $event, key)"
											:value="item"
										/>
										...
										<!-- </td> -->
										<table-cell
											@click.native="clicked(item, column, key)"
											:row="item"
											v-for="column in renderColumns"
											:class="{ sorting: sort.sortKey == column.key }"
											:column="column"
										></table-cell>
										<th class="last shrink" v-if="actionsEnabled">
                                            ...
                                            <fluro-icon icon="pencil" />
												...
												<v-btn
													...
													small
													icon
													@click.stop.prevent="$actions.open([item])"
												>
											...
											<fluro-help
												uid="tablerow"
												title="More Actions"
												body="View more actions related to this item"
											/>
										</th>
									</template>
								</tr>
				...
				<v-layout row wrap>
					...
					<template v-else>
						<v-flex xs6 align-center d-flex>
							<fluro-help
								title="Item count"
								body="Shows how many items are displayed in the table, clicking here will allow you to change the amount of rows displayed per page"
							/>
									...
									<template v-slot:activator="{ on }">
										<div v-on="on">
											<div v-if="groupingColumn">
												{{ totalGroups }} groups - Showing
												{{ (startOffset + 1) | numberWithCommas }} to
												{{ endOffset | numberWithCommas }} of
												{{ filteredTotal | numberWithCommas }} rows
											</div>
											<div v-else>
												{{ (startOffset + 1) | numberWithCommas }} to
												{{ endOffset | numberWithCommas }} of
												{{ filteredTotal | numberWithCommas }} {{ plural }}
											</div>
										</div>
									</template>
									<v-card tile>
										<v-list dense>
											<v-list-tile @click="perPage = 25">
												<v-list-tile-content>
													<v-list-tile-title> Show 25 per page </v-list-tile-title>
												</v-list-tile-content>
											</v-list-tile>
											<v-list-tile @click="perPage = 50">
												<v-list-tile-content>
													<v-list-tile-title> Show 50 per page </v-list-tile-title>
												</v-list-tile-content>
											</v-list-tile>
											<v-list-tile @click="perPage = 100">
												<v-list-tile-content>
													<v-list-tile-title> Show 100 per page </v-list-tile-title>
												</v-list-tile-content>
											</v-list-tile>
											<v-list-tile @click="perPage = 200">
												<v-list-tile-content>
													<v-list-tile-title> Show 250 per page </v-list-tile-title>
												</v-list-tile-content>
											</v-list-tile>
											<v-list-tile @click="perPage = 500">
												<v-list-tile-content>
													<v-list-tile-title> Show 500 per page </v-list-tile-title>
												</v-list-tile-content>
											</v-list-tile>
										</v-list>
									</v-card>
							...
							<div>
								<div class="pagenumber-select">
									<fluro-help
										...
										body="Displays the current page you are on, click here to quickly jump to a specific page"
									/>
									<span v-if="$vuetify.breakpoint.smAndUp" class="text-muted"
										>Page {{ currentPage }} of {{ totalPages }}</span
									>
									<select v-model="currentPage">
										<option :value="index + 1" v-for="(page, index) in availablePages">
											{{ index + 1 }}
										</option>
									</select>
								</div>
...
export default {
	...
	computed: {
		...
		reloadRequired() {
			return `${this.pageCacheKey}-${this.dataType}-${this.filterCheckString} ${this.dateWatchString} ${
				this.joins
			} ${this.sort.sortKey} ${this.sort.sortDirection} ${this.sort.sortType} ${
				this.groupingColumn ? this.groupingColumn.key : ''
	...
	methods: {
				...
				grouped = _.chain(rows)
					.reduce(function (set, row) {
						...
						function addGroupingEntry(groupingObjectValue) {
							var groupingKey =
								_.get(groupingObjectValue, '_id') ||
								_.get(groupingObjectValue, 'title') ||
								_.get(groupingObjectValue, 'name') ||
								_.get(groupingObjectValue, 'value') ||
								groupingObjectValue;
							var groupingTitle =
								_.get(groupingObjectValue, 'title') ||
								_.get(groupingObjectValue, 'name') ||
		...
		populatePageItems(rawPage, dataType, renderColumns) {
			...
			return new Promise(function (resolve, reject) {
				...
				self.$fluro.api
					.post(`/content/${dataType}/multiple`, pageRequest)
					.then(function (res) {
						//Cache the data
