<template>
	<div class="fluro-table-wrapper">
		...
		<v-container class="flex-center" v-if="!sorted.length">
			<slot name="emptytext"> No items </slot>
		</v-container>
		<template v-else>
			<div
				ref="scrollableArea"
				class="fluro-table-scroll"
				:class="{ scrollable: isScrollable, clickable: clickable, loading: showLoading }"
			>
				<span ref="top"></span>
								...
								<v-menu @click.native.stop offset-y attach>
									...
									<v-card tile style="margin: 0">
										<v-list dense>
											...
											<v-list-tile
												@click="deselectAll()"
												v-if="selectionManager.selection.length"
											>
												<v-list-tile-content>
													<v-list-tile-title>
							...
							<template v-for="group in grouped">
								...
								<tr :class="classes(item)" :key="item[trackingKey]" v-for="(item, key) in group.items">
									<template v-if="item._populating">
										...
										<th class="shrink" v-if="actionsAreEnabled"></th>
									</template>
									<template v-else>
										<th
											class="first"
											...
											@click.native.stop.prevent="checkboxClick(item, $event, item._pageIndex)"
											:value="item"
										/>
										<table-cell
											...
											:row="item"
											v-for="column in columns"
											:class="[column.classes, { sorting: sort.sortKey == column.key }]"
											:column="column"
										></table-cell>
										<th class="last shrink" v-if="actionsAreEnabled">
						...
						<tr v-if="totals && totals.length">
							...
							<template v-for="(column, index) in columns">
								...
								<table-cell
									v-if="totals[index]"
									v-tippy
									:content="`Average: ${avg[index]}`"
									:row="totals"
									:column="{ renderer: column.renderer, type: column.type, key: index }"
								/>
								<td v-else></td>
								<!-- <Span v-if="totals[index]" style="padding:8px;">{{totals[index]}}</Span> -->
								<!-- </th> -->
							</template>
							<th class="last shrink" v-if="actionsAreEnabled"></th>
						</tr>
					</tfoot>
				</table>
			</div>
			<div class="footer-stats" v-if="footerEnabled">
				<v-layout row wrap>
					<template v-if="!totalPages">
						<v-flex class="text-muted"> No matches </v-flex>
					</template>
					<template v-else>
	...
	methods: {
		totalTooltip(column, index) {
			return `${column.title} \nTotal: ${this.totals[index]} \nAverage: ${Math.round(
				(this.totals[index] || 0) / this.filtered.length
			)}`;
		},
