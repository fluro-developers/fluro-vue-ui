    ...
    <v-card class="chart-widget" v-if="hasData">
            ...
            <v-layout row wrap>
                ...
                <v-toolbar-items class="hidden-xs-only">
                    <v-menu :fixed="true" transition="slide-y-transition" offset-y>
                        ...
                        <v-list dense>
                            <v-list-tile @click="selectChartType(option)" v-for="option in chartTypeOptions" :key="option.value">
                                <!-- <v-list-tile-avatar> -->
                                <!-- </v-list-tile-avatar> -->
                                <v-list-tile-content>
                                    {{option.title}}
                    ...
                    <v-menu :fixed="true" transition="slide-y-transition" offset-y>
                        ...
                        <v-list dense>
                            <v-list-tile @click="selectDateFormat(option)" v-for="option in dateGroupFormatOptions" :key="option.value">
                            ...
                            <v-layout>
                                <v-flex>
                                    <v-card-text>
                                        <label>&nbsp;</label>
                                        <v-list dense>
                                            <v-list-tile @click="datePeriod = option" v-for="option in datePeriodOptions" :key="option.value">
                                                {{option.title}}
                                            </v-list-tile>
                                        </v-list>
                                    </v-card-text>
        ...
        <v-layout row wrap fill-height>
            <template v-if="trend.end" v-for="trend in enabledSeries.slice(0, 4)">
                <v-flex shrink>
                    <statbox :color="trend.color" :label="trend.title" :start="trend.start" :mean="trend.mean" :end="trend.end" />
                    <!-- {{trend.mean}} -->
        ...
        <div class="chart-wrapper-outer" :class="{empty:!enabledSeries.length}">
            ...
            <div class="chart-wrapper-inner">
                <apexchart ref="chart" :height="350" @dataPointSelection="markerClick" :type="chartType.value" :options="chartOptions" :series="enabledSeries" />
            </div>
        </div>
        ...
        <template v-if="series.length">
            <!-- <pre>{{enabledSeries}}</pre> -->
            <v-data-table :hide-actions="series.length <= 10" :headers="headers" :rows-per-page-items="perPageOptions" :items="series" class="elevation-0">
                <template v-slot:items="props">
                    <!--  -->
                    <tr :class="{inactive:!seriesIsActive(props.item)}">
                        <td @mouseover="showHighlight(props.item)" @mouseleave="hideHighlight()" @click="toggleSeries(props.item, $event.metaKey)">
...
export default {
    ...
    computed: {
        ...
        stroke() {
            ...
            _.each(self.enabledSeries, function(series, index) {
                output.colors[index] = _.get(series, 'options.stroke.color') || _.get(series, 'color') || self.theme.colors[index % self.theme.colors.length];
        ...
        fill() {
            ...
            _.each(self.enabledSeries, function(series, index) {
                output.colors[index] = _.get(series, 'options.fill.color') || _.get(series, 'color') || self.theme.colors[index % self.theme.colors.length];
            });
    ...
    methods: {
        ...
        mapSeries(title, key, color, options) {
                ...
                .map(function(group) {
                    ...
                    var totalledValue = _.chain(group.entries)
                        ...
                        .reduce(function(set, entry) {
                            ...
                            table += `<tr><td>${entry.title || ''}</td><td>${self.$fluro.date.formatDate(entry.date, 'h:mma D MMM YYYY')}</td><td><strong>${entryValue}</strong></td></tr>`;
            ...
            var mean = Math.round(_.chain(values)
                .compact()
            ...
            var max = _.max(_.map(values, function(array) {
                return array[1];
            }))

            var min = _.min(_.map(values, function(array) {
                return array[1];
            }))
