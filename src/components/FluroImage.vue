...
export default {
	...
	computed: {
		...
		computedWidth() {
			return parseFloat(
				this.imageWidth || this.loadedImageWidth || this.width || _.get(this.item, 'width') || 1
			).toFixed(4);
		},
		computedHeight() {
			return parseFloat(
				this.imageHeight || this.loadedImageHeight || this.height || _.get(this.item, 'height') || 1
			).toFixed(4);
		},
		aspectRatio() {
		...
		placeholderImage() {
			...
			if (self.imageID) {
				...
				var imageWidthCapped = 50;
				var imageHeightCapped = self.aspectRatio ? (self.aspectRatio / 100) * imageWidthCapped : null;

				console.log(imageWidthCapped, imageHeightCapped);
